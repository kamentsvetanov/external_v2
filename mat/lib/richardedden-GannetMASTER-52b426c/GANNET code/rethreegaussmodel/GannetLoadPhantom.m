function [MRS_struct] = GannetLoadPhantom(temppfile, waterfile)
...
for ii=1:numpfiles
    ...
    if(strcmpi(MRS_struct.vendor,'Siemens') ~= 1)
        ...
        %raee new code for 'over-tipped water'...looks for min - eventually need other
        %code for well-suppressed water, based on Cr signal.
        if Water_Positive ==1
        ...
        if(strcmpi(MRS_struct.vendor,'Philips_data'))
            ...
            CrSumSpecFit = FitPeaksByFramesPhantom(freqrange, CrSumSpec, Cr_initx);
            ...
            if(FreqPhaseAlign)
                ...
                [CrFitParams, rejectframes] = FitPeaksByFramesPhantom(freqrange, Cr_frames_OFF, Cr_initx);
                ...
                %This is hte phantom version and we didn't ealign - need to
                %join up:
                ...
                if(strcmpi(MRS_struct.vendor,'Philips_data'))
                    MRS_struct.Navg(ii) = MRS_struct.Navg(ii)*MRS_struct.nrows(ii) - numreject;
                else
    ...
    if(strcmpi(MRS_struct.vendor,'Philips_data'))
        ...
        MRS_struct.gabaspec = fftshift(fft(MRS_struct.ondata-MRS_struct.offdata,ZeroFillTo));
        MRS_struct.sumspec = fftshift(fft(MRS_struct.ondata+MRS_struct.offdata,ZeroFillTo));
    end
    end
