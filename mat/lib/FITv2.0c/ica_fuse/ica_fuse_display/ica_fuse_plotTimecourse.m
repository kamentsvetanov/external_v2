...
if exist('meanData', 'var')
    ...
    for nn = 1:size(meanData, 2)
        ...
        plot(xAxis, meanData(:, nn), meanDataColors{colorCount});
        hold on;
    end
    hold on;
end
...
if exist('axesHandle', 'var')
    if isempty(groupCompData)
        plot(xAxis, yAxis, time_course_color, 'parent', axesHandle);
        legendString{length(legendString) + 1} = ['Joint Component Signal'];
    else
        ...
        for nn = 1:size(meanData, 2)
            ...
            plot(xAxis, squeeze(groupCompData(:, 2, nn)), compDataColors{colorCount}, 'parent', axesHandle);
    ...
    plot_handle = plot(xAxis, yAxis, time_course_color);
...
if isempty(legendString)
    legendString{1} = ['Joint Component Signal'];
    %else
