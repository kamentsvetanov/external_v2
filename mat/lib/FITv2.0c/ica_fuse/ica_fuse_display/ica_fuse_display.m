...
if ~exist('fusionFile', 'var')
    fusionFile = ica_fuse_selectEntry('title', 'Select fusion information file for displaying joint ICA components', ...
...
if ~isfield(fusionInfo.run_analysis, 'outputFiles')
    error('No output joint ICA component images to display');
end
...
% Display GUI figure
[displayHandle] = ica_fuse_getGraphics('Display GUI For Joint ICA', 'displaygui', displayTag, 'off');
...
axis(titleAxesH, 'off');
xPos = 0.5; yPos = 0.97;
text(xPos, yPos, 'Display GUI For Joint ICA', 'color', titleColor, 'FontAngle', 'italic', 'fontweight', 'bold', ...
...
textToPlot = {'Do You Want To Sort Joint ICA Components?'};
...
try
    ...
    if (length(selectedComp) == numComp) & strcmpi(sortSelString, 'no')
        ...
        if ~isempty(eegIndices)
            ...
            disp(['Ordering joint ICA components based on peak of feature ', outputFiles(eegIndex).feature_name, ' ...']);
            fprintf('\n');
            % Load eeg data
