...
if isempty(mask_files)
    ...
    for nFeature = 1:numFeatures
        ...
        elseif strcmpi(featureInfo(nFeature).modality, 'gene')
...
elseif strcmpi(modality, 'gene')
    
    dispStr = ['Using Gene mask for feature ', featureInfo.feature_name];
    disp(dispStr);
    if isempty(mask_file)
        error(['Gene mask for feature ', featureInfo.feature_name, ' is empty']);
    end
    fprintf('\n');
    maskData = str2num(mask_file);
