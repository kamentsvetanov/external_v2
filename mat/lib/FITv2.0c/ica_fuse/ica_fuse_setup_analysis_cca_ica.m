...
if isfield(fusionInfo.setup_analysis, typePCATag)
    ...
    inds = strmatch(lower(pcaStr), char('cca', 'mcca'), 'exact');
    if (isempty(inds))
        inds = 1;
    end
...
switch (inputTag)
    ...
    case 'estimate_components'
        ...
        D(1).string = 'For MRI data-sets, estimation code is based on i.i.d sampling and Minimum Description Length (MDL) is applied after estimating the samples. For other modalities, standard MDL estimation is used.';
    ...
    case 'type_pca'
        ...
        D(1).string = 'There are two options like ''CCA'' and ''MCCA''.';
        D(length(D) + 1).string = '';
    ...
    case 'algorithm'
        ...
        D(1).string = 'Currently, 10 ICA algorithms are available in the toolbox like Infomax, Fast ICA, Erica, Simbec, Evd, Jade Opac, Amuse, SDD ICA, CCICA and Combi.';
