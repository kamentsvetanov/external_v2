function [aveComp, loadingCoeff, maxcorr, maxrow, sR1, sR2] = ica_fuse_calculate_parallelICA(data, dewhiteM, whiteM, ...
...
if (strcmpi(analysisType, 'average'))
    ...
    for run = 1:numICARuns
        ...
        if strcmpi(type_parallel_ica, 'aa')
            ...
            [W, sphere, icasig_tmp, bias, signs, lrates, y] = ica_fuse_runica_parallelicaMul_AA(data, 'dewhitem', ...
                dewhiteM, 'whitem', whiteM, ICA_Options{:});
        else
            disp('Using parallel ICA algorithm AS ..');
            [W, sphere, icasig_tmp, bias, signs, lrates, y] = ica_fuse_runica_parallelica_AS(data, 'dewhitem', ...
                dewhiteM, 'whitem', whiteM, ICA_Options{:});
        end
