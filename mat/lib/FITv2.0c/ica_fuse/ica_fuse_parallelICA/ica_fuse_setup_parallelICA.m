...
try
    ...
    if ~strcmp(getSubjects, 'yes')
        ...
        for nn = 1:length(dataInfo)
            modality1_files(nn).name = str2mat(dataInfo(nn).feature(1).files.name);
...
featureNames = cellstr(str2mat(paraICAInfo.setup_analysis.dataInfo(1).feature.name));
modalities = cellstr(str2mat(paraICAInfo.setup_analysis.dataInfo(1).feature.modality));
...
try
    ...
    groupNames = str2mat(paraICAInfo.setup_analysis.dataInfo.name);
    numSubjects = paraICAInfo.setup_analysis.numSubjects;
...
if strcmpi(deblank(getStr(getVal, :)), 'aa')
...
function showHelpDialog(inputTag)
...
switch (inputTag)
    ...
    case 'modality1_numComp'
        
        titleStr = 'Num of PC for feature 1';
        D(1).string = 'Enter number of principal components to be extracted from the data for feature 1.';
        
    case 'modality2_numComp'
        
        titleStr = 'Num of PC for feature 2';
        D(1).string = 'Enter number of principal components to be extracted from the data for feature 2.';
        
    case 'type_parallel_ica'
        
        titleStr = 'Type of parallel ICA';
        D(1).string = 'There are two options ''AA'' and ''AS''.';
        D(length(D) + 1).string = '';
    ...
    case 'estimate_components'
        ...
        D(1).string = 'Dimensionality estimation is done for each feature using Minimum Description Length (MDL) criteria. Estimation information is stored in the fusion information MAT file. To access the information, please see variable paraICAInfo.setup_analysis.estimationInfo';
