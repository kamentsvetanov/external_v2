...
%
    ...
    h.btnChooseFile=uicontrol(h.panelFile,'Style','pushbutton', ...
        'String','Choose IBI File',...
        'Units', 'normalized', 'Position',[.845 .08 .1 .8],...
        'Callback', @btnChooseFile_Callback);    
    h.btnRun=uicontrol(h.panelFile,'Style','pushbutton', 'String','Run',...
        'Units', 'normalized', 'Position',[.947 .08 .05 .8],...
        'Callback', @btnRun_Callback);
...
%% GUI: Results - Time Domain Tab
    ...
    h.tab1 = uitab('v0',h.tabgroup, 'title', 'Time Domain','Position',[1 1 1 1]);
    h.panelTime = uipanel('Parent',h.tab1,'Position',[.0 .0 1 1], ...
        'BackgroundColor','white');    
    h.axesHistIBI=axes('parent', h.panelTime,'Position',[.05 .08 .4 .28], ...
        'FontSize',7,'Box','on');
    xlabel(h.axesHistIBI,'IBI (ms)'); title(h.axesHistIBI,'IBI Histogram')
...
%% GUI: Results - Freq Doamin Tab
    ...
    h.tab2 = uitab('v0',h.tabgroup, 'title', 'Freq Domain','Position',[10 1 1 1]);
...
%% GUI: Results - Poincare Tab
    ...
    h.tab3 = uitab('v0',h.tabgroup, 'title', 'Poincare','Position',[10 1 1 1]);
...
%% Helper and Utility Functions
    ...
    % createTimeTbl: creats the table to display time-domain HRV results    
        ...
        %Column 1
        tH(1,1)=text(x1,y(1),'MeanIBI','Parent',aH,'Units','normalized','FontSize',8);
        tH(2,1)=text(x1,y(2),'SDNN','Parent',aH,'Units','normalized','FontSize',8);
        tH(3,1)=text(x1,y(3),'MeanHR','Parent',aH,'Units','normalized','FontSize',8);
        tH(4,1)=text(x1,y(4),'SDHR','Parent',aH,'Units','normalized','FontSize',8);
...
%% GUI: CreateTFTbl
    ...
    % proccessing
        ...
        output.freq=freqDomainHRV(dIBI,opt.VLF,opt.LF,opt.HF,opt.AROrder,...
                    opt.WinWidth,opt.WinOverlap,opt.Points,opt.Interp);            
        
        %Nonlinear (using non-detrented ibi)
        showStatus('< Nonlinear >');
        output.nl=nonlinearHRV(nIBI,opt.m,opt.r,opt.n1,opt.n2,opt.breakpoint);        
        %Poincare
        showStatus('< Poincare >');
