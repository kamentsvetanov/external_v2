...
[OL_posterior,OL_out] = VBA_NLStateSpaceModel(...
    OL_y,OL_u,f_fname,g_fname,dim,OL_options);
...
if ~options.microU
    ...
    u0 = u;
    out.u = zeros(dim.u,options.decim*dim.n_t);
    for t=1:dim.n_t
        ut = reshape(u0(:,t),dim.u,options.decim);
        out.u(:,(t-1)*options.decim+1:t*options.decim) = ut;
    end
end
out.y = y;
out.dim = dim;
...
if OL_out.dim.n_phi >= 1
    ...
    suffStat.dphi(:,t) = -posterior.muPhi(:,t)+options.priors.muPhi;
...
if OL_out.dim.n_theta >= 1
    ...
    suffStat.dtheta(:,t) = -posterior.muTheta(:,t)+options.priors.muTheta;
...
suffStat.F = [suffStat.F,suffStat.F + OL_out.F]; 
