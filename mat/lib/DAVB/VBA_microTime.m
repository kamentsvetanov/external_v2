...
for t=1:options.dim.n_t
    ...
    for i=1:options.decim
        ...
        [x(:,indT)] = feval(options.f_fname,x(:,indT-1),...
            theta,u(:,tu),options.inF);
        if ~isempty(out.suffStat.dx) && isequal(i,options.decim)
            % add state noise to evolution mapping
            x(:,indT) = x(:,indT) + out.suffStat.dx(:,t);
        end
        [gx(:,indT)] = feval(options.g_fname,x(:,indT),...
            phi,u(:,tu),options.inG);
    end
end
