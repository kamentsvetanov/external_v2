function [posterior,out] = VBA_NLStateSpaceModel(...
    y,u,f_fname,g_fname,dim,options,in)
...
%       .checkGrads: a flag for checking provided analytical gradients of
...
%       .finalEval: a string which will be evaluated at the bottom end of
%       this main stream function. This may be useful for retrieving
%       internal information from the routine {[]}
%       .EvoEval: externally-specified function evaluated after each VB
%       update of the evolution parameters {[]}
%       .ObsEval: externally-specified function evaluated after each VB
%       update of the observation parameters {[]}
%       .StatesEval: externally-specified function evaluated after each VB
...
%       .noSXi: flag for zeroing the hidden-states lagged-covariance matrix
...
%       .finalEval : this is used to store the result of the evaluation of
%       the function in.finalEval if it exists.
...
if exist('in','var')
    ...
    try  % Skip checking and replace prior initialization
        ...
        if ~options.OnLine
            if isequal(options.g_fname,@VBA_odeLim)
                VBA_disp(...
                    'Initialization: non-stochastic system (ODE).',...
                    options)
            else
        ...
        error('The ''in'' structure was flawed...')
    end
    ...
    [posterior,suffStat,options] = ...
        VBA_Initialize(y,u,f_fname,g_fname,dim,options);
...
try
    delete(intersect(findobj('tag','diagnostics_tabs'),...
        get(options.display.hfp,'children')));
end
...
while ~stop
    ...
    if dim.n > 0
        ...
        [posterior,suffStat] = ...
            VBA_GN(y,posterior,suffStat,dim,u,options,'X');
        % Call for VB initial condition update.
        if options.updateX0
            [posterior,suffStat] = ...
                VBA_GN(y,posterior,suffStat,dim,u,options,'X0');
        end
        % Call externally-specified function
        if ~isempty(options.StatesEval)
            try eval(options.StatesEval); end
        end
    end
    ...
    if dim.n_phi > 0
        ...
        [posterior,suffStat] = ...
            VBA_GN(y,posterior,suffStat,dim,u,options,'Phi');
        % Call externally-specified function
        if ~isempty(options.ObsEval)
            try eval(options.ObsEval); end
        end
    end
    ...
    if dim.n_theta > 0
        ...
        % parameters. This updates the posterior and suffStat variables
        [posterior,suffStat] = ...
            VBA_GN(y,posterior,suffStat,dim,u,options,'Theta');
        % Call externally-specified function
        if ~isempty(options.EvoEval)
            try eval(options.EvoEval); end
        end
    end
    ...
    if options.updateHP
        [posterior,suffStat] = VBA_VBVarParam(...
            y,posterior,suffStat,dim,options);
    ...
    % Display progress
    VBA_updateDisplay(...
        suffStat.F,posterior,suffStat,options,y,it,'precisions')
...
% Evaluates the string command passed in the options structure.
if ~isempty(options.finalEval)
    try
        eval(options.finalEval)
    catch
        VBA_disp(...
            'Warning: could not evaluate ''options.finalEval'' string.',...
            options)
        out.finalEval = '?! could not evaluate options.finalEval ?!';
    end
end
...
if ~options.OnLine
    ...
    mins = floor(dt./60);
    if mins >= 1
        str = ['VB ',status,' complete (took ~',num2str(mins),' min).'];
    else
        str = ['VB ',status,' complete (took <1 min).'];
    end
