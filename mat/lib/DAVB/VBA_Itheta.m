function [Itheta,SigmaTheta,deltaMuTheta,suffStat] = VBA_Itheta(...
    theta,y,posterior,suffStat,dim,u,options)
...
[fx,dF_dX,dF_dTheta,d2F_dXdTheta] =...
    VBA_evalFun('f',posterior.muX0,Theta,u(:,1),options,dim);

% check infinite precision transition pdf
iQ2 = VB_inv(iQx{1},indInx{1},'replace');

% mean-field terms
SXd2fdx2 = trace(dF_dX*iQ2*dF_dX'*posterior.SigmaX0);
...
for t=1:dim.n_t-1
    ...
    % evaluate evolution function at current mode
    [fx,dF_dX,dF_dTheta,d2F_dXdTheta] =...
        VBA_evalFun('f',posterior.muX(:,t),Theta,u(:,t+1),options,dim);    
    
    % mean-field terms
    SXd2fdx2 = SXd2fdx2 ...
        + trace(dF_dX*iQ2*dF_dX'*posterior.SigmaX.current{t});
    SXtdfdx = SXtdfdx ...
        -2*trace( iQ2*dF_dX'*posterior.SigmaX.inter{t} );
    Sthetad2fdtheta2 = Sthetad2fdtheta2 ...
        + trace(dF_dTheta*iQ2*dF_dTheta'*posterior.SigmaTheta);
    ...
    if isweird(dx2) || isweird(dF_dX) || isweird(dF_dTheta)
        div = 1;
        break
    end
...
if isweird(Itheta) || isweird(SigmaTheta) || div
    Itheta = -Inf;
end
