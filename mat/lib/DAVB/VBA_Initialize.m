...
% function
% [posterior,suffStat] = VBA_Initialize(y,u,muX0,f_fname,g_fname,options)
%
...
if dim.n > 0        % if any hidden-states to be estimated
    ...
    if dim.n_phi + dim.n_theta > 0
        ...
        options0 = options;
        options0.figName = ...
            'VB-Laplace initialization: non stochastic system';
        options0.priors.a_alpha = Inf;
        options0.priors.b_alpha = 0;
%         options0.GnMaxIter = 1;
%         options0.MaxIter = 1;

        if ~options.priors.AR
            [in.out.options,u,in.out.dim] = VBA_check(...
                y,u,options0.f_fname,options0.g_fname,dim,options0);
        ...
        [in.posterior,in.out.suffStat,in.out.options] = ...
            VBA_Initialize(y,u,f_fname,g_fname,in.out.dim,in.out.options);
        in.out.F = VBA_FreeEnergy(...
            in.posterior,in.out.suffStat,in.out.options);
        in.out.u = u;
        ...
        if options.init0
            ...
            [posterior.muX,posterior.SigmaX.current,suffStat] = ...
                VBA_EKF(y,u,posterior,dim,options,2);
    ...
    posterior.SigmaX.inter = cell(dim.n_t,1);
    In = speye(dim.n);
    for t = 1:dim.n_t-1
    ...
    opt0 = options;
    opt0.DisplayWin = 0;
    [o1,o2,o3,suffStat] = VBA_IX(...
        posterior.muX,y,posterior,suffStat,dim,u,opt0);
    ...
    if ~options.binomial
        ...
        [o1,o2,o3,suffStat] = VBA_Iphi(...
            posterior.muPhi(indIn),y,posterior,suffStat,dim,u,opt);
