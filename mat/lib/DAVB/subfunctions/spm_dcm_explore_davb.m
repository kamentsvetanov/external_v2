...
% buikd GUI for DCM results reviewing
...
try
    [pathstr, filename, ext, versn] = fileparts(DCM.filename);
...
str{3} = sprintf(['Dimensions of the model:','\n ',...
    ...
    '    - estimated parameters: n_theta=',num2str(np),'\n ']);
str{4} = sprintf(['EPI repetition time: TR=',num2str(DCM.Y.dt),'\n',...
    'EPI echo time: TE=',num2str(DCM.TE),'\n']);
if ~DCM.options.stochastic
    str{5} = sprintf('This was a deterministic DCM');
else
    str{5} = sprintf('This was a stochastic DCM');
end
if DCM.options.two_state
    str{5} = sprintf([str{4},' (two states per region):']);
else
    str{5} = sprintf([str{4},' (one state per region):']);
end
gfn = DCM.M.g;
ffn = DCM.M.f;
ifn = DCM.M.IS;
str{65} = sprintf(['    - observation function: ',gfn,'\n',...
...
if isfield(DCM,'xY')
    ...
    for i = 1:length(DCM.xY)
        ...
        text(0,y,name,...
            ...
            'FontSize',12,...
            'parent',ha)
        text(1,y,sprintf('%0.0f',N),...
            'FontSize',12,...
            'parent',ha)
        text(2,y,sprintf('%-4.0f %-4.0f %-4.0f',L),...
            'FontSize',12,...
            'parent',ha)
        y = y - 1;
    end
...
imagesc(abs(ud.Pcorr),'parent',ha)
try
    set(ha,...
        'xtick',ltick,...
        'ytick',ltick,...
...
axis(ha,'square')
title(ha,...
    'Parameters (absolute) posterior correlation matrix','FontSize',12)
set(ha,'clim',[-1/32 1]);
col = colormap('jet');
col(1,:) = 0.5*ones(1,3);
colormap(ha,col);
...
title(ha,[matType,' - fixed effects'],'FontSize',12)
...
title(ha(2),[matType,' - posterior probabilities: P(|effect|>0)'],'FontSize',12)
if isequal(matType,'C')
    set(ha(2),...
        'XTick',[1:dim.m],'XTickLabel',DCM.U.name,...
        'YTick',[1:dim.l],'YTickLabel',DCM.Y.name)
