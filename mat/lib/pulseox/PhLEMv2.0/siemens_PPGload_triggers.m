function [signal time] = siemens_PPOload_triggers(fname,varargin);
...
indx = setdiff(data_t(:),t_off); %Note: depending on when the scan ends, the last size(t_off)~=size(t_on).
signal = data{1}(indx);
...
if TR < 1000
  ...
  start = StartClip*(Hz/1000)+Hz/2+(TR*SkipTR)*(Hz/1000);
  stop = EndClip*(Hz/1000)+mod(length(signal(1:end)),Hz)+ Hz/2;
...
% Comment if willing to keep all timeseries kat 20130428
