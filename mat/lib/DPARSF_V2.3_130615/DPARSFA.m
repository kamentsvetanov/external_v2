...
function DPARSFA_OpeningFcn(hObject, eventdata, handles, varargin)
    ...
    handles.Cfg.Covremove.Timing='AfterRealign';  %Another option: AfterNormalizeFiltering
    ...
    if ~(exist('rest.m')&&exist('spm.m'))
        uiwait(msgbox('DPARSFA is based on SPM and REST, Please install Matlab 7.3, SPM5, REST V1.8 or later version at first.','DPARSFA'));
    else
...
function UpdateDisplay(handles)
    ...
    if handles.Cfg.IsCovremove==1
        if strcmpi(handles.Cfg.Covremove.Timing,'AfterRealign')
            ...
            set(handles.checkboxCovremoveAfterNormalizeFiltering, 'Value', 0, 'ForegroundColor', [0.4,0.4,0.4]);
        ...
        elseif strcmpi(handles.Cfg.Covremove.Timing,'AfterNormalizeFiltering')
