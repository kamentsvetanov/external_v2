...
%Based on the interfaces of rest_bandpass orignially written by Xiao-Wei Song
...
if ~isempty(AMaskFilename)
    [MaskData,MaskVox,MaskHead]=rest_readfile(AMaskFilename);
else
...
y_Write4DNIfTI(AllVolumeBrain,Header_Out,sprintf('%s_filtered%sFiltered_4DVolume.nii', ADataDir, filesep));
