function [SPM,xSPM] = wfu_spm_getSPM2_conj
...
%_______________________________________________________________________
% @(#)spm_getSPM.m	2.50 Andrew Holmes, Karl Friston & Jean-Baptiste Poline 03/03/04
% Conjunction analysis from: 
% spm_getSPM.m,v 1.14 2004/10/19 15:41:56 nichols Exp $ UM Biostat
...
if spm_input('ROI Analysis','+1','y/n',[1,0],2)
	...
	if ~isempty(wfu_atlas_filename)
		P = spm_get('Files',swd,'beta*.img');   %get betas
                if isempty(P)
		    P = spm_get('Files',swd,'spm*.img');   %get spmT, spmF, spmC
                end
                if isempty(P)
		    error('WFU_PickAtlas requires beta or spm image for realignment');
                end
		P = {char(P(1,:)),wfu_atlas_filename};  %create list for realignment
		flags.mean=0;               
		flags.hold = 0;
		flags.which=1;
		flags.mask=0;
		spm_reslice(P,flags);                   %reslice mask with betas  
		wfu_atlas_filename=prepend(wfu_atlas_filename,'r'); %use r*atlas.img
	end
...
if ~isempty(wfu_atlas_filename)
	...
	if any(find(abs(tM([2:5,7:10,12]))>=10^-6))
