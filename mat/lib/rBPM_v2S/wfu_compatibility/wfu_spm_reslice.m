function wfu_spm_reslice(P,flags,wfu_MT)
...
%--------------------------------------------------------------------------
%modifications made for object2target transformations
%SPM2/SPM5 compatibility (checks dt field)
%passes appropriate wfu structure for SPM99 compatibility  JAM 10-31-05
%--------------------------------------------------------------------------
...
%------------------------------------------------------------
%modification for object2target transformations  JAM 10-31-05
%------------------------------------------------------------
if exist('wfu_MT')
	for i = 2:length(P), P(i).mat = P(1).mat * wfu_MT; end;
end
...
for i = 1:prod(size(P)),
	...
	if read_vol,
		if write_vol,
			...
			VO.dim     = P(1).dim;
			if ~isfield(VO,'dt'), VO.dim     = [P(1).dim(1:3) P(i).dim(4)]; end;
			VO.mat     = P(1).mat;
			VO.descrip = 'spm - realigned';
			VO.wfu = [];
			if isfield(P(1),'wfu'), VO.wfu = P(1).wfu; end
		...
		if write_vol & ~isfield(VO,'dt'), VO = spm_close_vol(VO); end;
		nread = nread + 1;
	end;
