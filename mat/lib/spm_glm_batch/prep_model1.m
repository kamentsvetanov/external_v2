...
if robust_regression_setup
    ...
    for i = 1:length(input_contrasts)
        ...
        if isempty(EXPTm.SNPM.P{i})
            diary(diaryname), fprintf('robust reg setup failed: no contrast images...%s\n', modeldir), diary off
        end
...
%             robust_results_batch('mask', maskimg, 'thresh', [.001 .005 .05], 'size', [5 1 1], 'prune');
...
% 2 - warping check - scnlab_norm_check3  ( and coreg check)
% 3 - contrast maps before robust reg - on same scale, same
% shape/orientation, missing values

% design check: x axis on power plots,
% text: vifs in full design, mask check if exists?

% slide with flowchart and dir structure
...
% NOTES:
% spm will convert each session to global mean of 100 by default (approx,
% based on loose mask)
% if you are modeling multiple runs as one big session, session (run)-specific
% scaling will not be implemented. you may wish to do this...or not...

% spm high-pass filters data first, then models.  any shared variance will
% be attributed to noise.


% ROBUST REG: Save weights, Save contrast name, Analysis ignoring NaNs
% voxel-wise
