...
%% Re-do nuisance regressors for this model - remove movement regs

nuisancedir = fullfile(basedir, 'nuisance');
nuisancesavedir = fullfile(basedir, 'first_level', modelname, 'nuisance');
mkdir(nuisancesavedir)
...
for i = subjs
    ...
    clear nuisance
    load(nuisancefile, 'nuisance');
    R = nuisance.intercept;
    
    nuisancesavefile = fullfile(nuisancesavedir, [EXPT.subjects{i} '_nuisance.mat']);
    save(nuisancesavefile, 'R');
...
if build_spm_batch_job
    ...
    for i = subjs
        ...
        % Nuisance regressors
        % *** EDIT here to change where filename for nuisance regressors are saved/which files are used
        nuisancefile = fullfile(basedir, 'first_level', modelname, 'nuisance', [EXPT.subjects{i} '_nuisance.mat']);
