function Pphi=Pmutualinf(y,sr,lambdamin,lambdamax)

if  nargin<2, lambdamin=0;lambdamax=1/2; sr=1;end
...
for p1=1:m
        for p2=(p1+1):m
            Pphi(p1,p2)=(1-exp(-2*delta(p1,p2)))^.5;
        end 
end
Pphi=Pphi+Pphi';%+diag(diag(ones(m)));