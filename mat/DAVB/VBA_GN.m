function [posterior,suffStat] = ...
    VBA_GN(y,posterior,suffStat,dim,u,options,flag)
...
% posterior:
try
    [I,Sigma,deltaMu,suffStat2] = feval(fname,...
        PreviousMu,y,posterior,suffStat,dim,u,options);
    PreviousI = I;
...
while ~stop
    ...
    try
        ...
        [I,Sigma,NextdeltaMu,suffStat2] = feval(fname,...
            mu,y,posterior,suffStat,dim,u,options);
        % get increment in variational/free energy
        [rdf,deltaI,F] = getCostIncrement(...
            I,PreviousI,mu,Sigma,suffStat2,options,posterior,flag,it);
    catch
        rdf = -1;
        deltaI = -Inf;
    end
    ...
    if it <= options.GnMaxIter && abs(rdf)> options.GnTolFun
        str = [s1,num2str(I,'%4.3e'),...
            ' ,dI/I =',num2str(rdf,'%4.3e'),...
            ' ,it #',num2str(it)];
        if deltaI<0     % halve step size
            ...
            switch flag
                case {'X','X0'}
                    VBA_updateDisplay(...
                        suffStat.F,posterior,suffStat,options,y,[],'X')
                case 'Phi'
                    VBA_updateDisplay(...
                        suffStat.F,posterior,suffStat,options,y,[],'phi')
                case 'Theta'
                    VBA_updateDisplay(...
                        suffStat.F,posterior,suffStat,options,y,[],'theta')
            end
...
function [rdf,deltaI,F] = getCostIncrement(...
    I,PreviousI,mu,Sigma,suffStat2,options,posterior,flag,it)
if options.gradF
    if ~isinf(I)
        switch flag
