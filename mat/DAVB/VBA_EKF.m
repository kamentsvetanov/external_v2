...
if flag>=1
    ...
    if flag == 1    % EKF update
        [gx(:,1),dG_dX] ...
            = VBA_evalFun('g',mStar(:,1),phi,u(:,1),options,dim);
        iRp = pinv(Rp);
        C =  dG_dX*iQy{1}*dG_dX';
        iSX = iRp + sigma*C;
        SigmaX{1} = pinv( iSX );
        muX(:,1) = mStar(:,1) ...
            + sigma.*SigmaX{1}*dG_dX*iQy{1}* (y(:,1)-gx(:,1));
    ...
    [gx(:,1),dG_dX] ...
        = VBA_evalFun('g',muX(:,1),phi,u(:,1),options,dim);
    suffStat.vy(:,1) = ...
        diag( sigma.^-1.*pinv(iQy{1}) + dG_dX'*SigmaX{1}*dG_dX );
    suffStat.dx(:,1) = muX(:,1) - fx0;
    suffStat.dx2 = suffStat.dx2 + ...
        suffStat.dx(:,1)'*iQx{1}*suffStat.dx(:,1);
    suffStat.dy(:,1) = y(:,1) - gx(:,1);
    suffStat.dy2 = suffStat.dy2 + ...
        suffStat.dy(:,1)'*iQy{1}*suffStat.dy(:,1);
    suffStat.SX = suffStat.SX ...
        + 0.5*sum(log(eig(full(SigmaX{1}))));
    suffStat.trSx = suffStat.trSx ...
        + trace(SigmaX{1});
else
    muX(:,1) = ...
        VBA_evalFun('f',X0,theta,u(:,1),options,dim);
...
for t = 1:dim.n_t-1
    if flag >= 1
        %-- Prediction
        [fx,dF_dX] ...
            = VBA_evalFun('f',muX(:,t),theta,u(:,t+1),options,dim);
        mStar(:,t+1) = fx;
        Rp = dF_dX'*SigmaX{t}*dF_dX + 1./alpha.*VB_inv(iQx{t+1},[]);
        if flag == 1    % EKF update
            [gx(:,t+1),dG_dX] ...
                = VBA_evalFun('g',mStar(:,t+1),phi,u(:,t+1),options,dim);
            C =  dG_dX*iQy{t+1}*dG_dX';
            iRp = pinv(Rp);
            iSX = iRp + sigma*C;
            SigmaX{t+1} = pinv( iSX );
            muX(:,t+1) = mStar(:,t+1) ...
                + sigma.*SigmaX{t+1}*dG_dX*iQy{t+1}* (y(:,t+1)-gx(:,t+1));
        else
            muX(:,t+1) = mStar(:,t+1);
            SigmaX{t+1} = Rp;
        end
        % get predicted observation at the mode
        [gx(:,t+1),dG_dX] ...
            = VBA_evalFun('g',muX(:,t+1),phi,u(:,t+1),options,dim);
        suffStat.vy(:,t+1) = ...
            diag( sigma.^-1.*pinv(iQy{t+1}) + dG_dX'*SigmaX{t+1}*dG_dX );
        suffStat.dx(:,t+1) = muX(:,t+1) - fx;
        suffStat.dx2 = suffStat.dx2 + ...
            suffStat.dx(:,t+1)'*iQx{t+1}*suffStat.dx(:,t+1);
        suffStat.dy(:,t+1) = y(:,t+1) - gx(:,t+1);
        suffStat.dy2 = suffStat.dy2 + ...
            suffStat.dy(:,t+1)'*iQy{t+1}*suffStat.dy(:,t+1);
        suffStat.SX = suffStat.SX ...
            + 0.5*sum(log(eig(full(SigmaX{t+1}))));
        suffStat.trSx = suffStat.trSx ...
            + trace(SigmaX{t+1});
    else
        muX(:,t+1) = ...
