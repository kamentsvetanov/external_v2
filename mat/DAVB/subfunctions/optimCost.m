...
% OUT:
%   - mu: the optimal x
%   - curv: the curvature of costFcn evaluated at opt
%   - out: a structure containing:
%       .it: the number of iterations of the regularized Gauss-Newton
%       optimization scheme
%       .conv=1 if optimization has converged, conv=0 if not.
...
dIdx = (-1)^minimize.*numericDiff(@myfun,1,x,myFun,minimize,args);
d2Idx2 = numericDiff(@numericDiff,3,@myfun,1,x,myFun,minimize,args);
S = -(-1)^minimize.*pinv(full(d2Idx2));
dx = S*dIdx;
