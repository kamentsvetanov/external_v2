function [X,m,S,It,diff,Xrec]=pcambpmp(X,A,maxiter,tol)
...
while It<maxiter & diff>tol,
  ...
  for i=1:n,            % for each row
    if pat(i).nM>0,     % if there are missing values
      L=V(pat(i).O,1:min(A,pat(i).nO));        % P*
      LL=V(pat(i).M,1:min(A,pat(i).nO));       % P#
      z1=Xc(i,pat(i).O)';     % xi*
      z2=LL*pinv(L'*L)*L'*z1; % xi#
      Xc(i,pat(i).M)=z2';     % [xi* xi#]
    end
  end
  X=Xc+ones(n,1)*mX;
  d=(X(mis)-Xmis).^2;
