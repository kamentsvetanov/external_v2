...
if ~isempty(log_files.cardiac)
    ...
    if hasScanTimingDicomImage
        ...
        % load dicom % Amended by kt03 to handle camcan dicoms
        dicomHeader(1)   = spm_dicom_headers(log_files.scan_timing(1,:));
        dicomHeader(2)    = spm_dicom_headers(log_files.scan_timing(end,:));

        ScanStartTimeSeconds    = dicomHeader{1}.AcquisitionTime;
        ScanStopTimeSeconds     = dicomHeader{2}.AcquisitionTime + ...
                                  dicomHeader{2}.RepetitionTime/1000;
        ...
        disp(['Phys log start   : ' secs2hms(LogStartTimeSeconds)]);
        disp(['Phys log stop    : ' secs2hms(LogStopTimeSeconds)]);
        disp(['Phys log duration: ' secs2hms(LogStopTimeSeconds-LogStartTimeSeconds)]);
        disp(['--- Acquisition information ---']);
        disp(['First acq dicom  : ' secs2hms(ScanStartTimeSeconds)]);
        disp(['Last acq dicom   : ' secs2hms(ScanStopTimeSeconds)]);
        disp(['Phys-dicom       : ' secs2hms(ScanStartTimeSeconds-LogStartTimeSeconds)]);
        disp(['Session Duration : ' secs2hms(ScanStopTimeSeconds-ScanStartTimeSeconds)]);
%         disp(['Phys indices     : ' num2str(acq_start_i) ' ... ' num2str(acq_stop_i)]);
    else
    ...
    lineData = C{1}{1};
    % We dont have trigger values 6002 in Cam-CAN,so next 2 lines ignored
...
%     lineData = lineData((iTrigger(end)+4):end);
    ...
    data = data{1}(5:end); % Remove first 4 digits from puls file. kat
    % Remove the systems own evaluation of triggers.
    cpulse        = find(data == 5000);  % System uses identifier 5000 as trigger ON
    cpulse_off    = find(data == 6000); % System uses identifier 5000 as trigger OFF
    recording_on  = find(data == 6002);% Scanner trigger to Stim PC?
    recording_off = find(data == 5003);
    ...
    iDataStream = 1:numel(data);
    iDataStream(iNonEcgSignals) = [];
    
    nSamples = numel(data_stream);
    nRows = ceil(nSamples);% kat changed from nRows = ceil(nSamples/2)
    ...
    data_table(1:nRows,1) = data_stream(1:end);% kat changed from data_table(1:nRows,1) = data_stream(1:2:end);
    iData_table(1:nRows,1) = iDataStream(1:end);% kat changed from iData_table(1:nRows,1) = iDataStream(1:2:end);
    
    if mod(nSamples,2) == 1
        data_table(1:nRows-1,2) = data_stream(1:end-1);% kat changed from data_table(1:nRows-1,2) = data_stream(2:2:end);
        iData_table(1:nRows-1,2) = iDataStream(1:end-1);% kat changed from iData_table(1:nRows-1,2) = iDataStream(2:2:end)
    else
        data_table(1:nRows,2) = data_stream(1:end);% kat changed from data_table(1:nRows,2) = data_stream(2:2:end)
        iData_table(1:nRows,2) = iDataStream(1:end);% kat changed from  iData_table(1:nRows,2) = iDataStream(2:2:end)
    end
    ...
    nSamples = size(c,1);
%     dt = dt*2; % Added temporarily to fix a bug where fs was halfed by splitting the data into 2 channels (lines 202-204) kat 20160311
