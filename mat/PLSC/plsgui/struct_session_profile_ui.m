%STRUCT_SESSION_PROFILE_UI Form to input the group info, including session name,
%		working directory, subjects directories, conditions names etc.
%
%   Usage: struct_session_profile_ui
%
...
function struct_session_profile_ui(varargin)

   if nargin == 0,

      if exist('plslog.m','file')
         plslog('Edit STRUCT Session');
      end
...
%      cd(start_dir);
   ...
   elseif strcmp(action,'SELECT_PLS_DATA_DIR'),
      SelectPLSDataDir;
   elseif strcmp(action,'EDIT_DATASET_DIR'),
      EditDatasetDir;
   elseif strcmp(action,'SELECT_DATASET_DIR'),
      SelectDatasetDir;
   ...
   elseif strcmp(action,'EDIT_CONDITIONS'),
      is_dir_empty = isempty(getappdata(gcf,'SessionPLSDir'));
      is_dir_empty = isempty(getappdata(gcf,'SessionDatasetDir'));
      if is_dir_empty
         msg = 'A working directory needs to be specified before inputting conditions.';
         msg = 'The dataset directory needs to be specified before inputting conditions.';
         set(findobj(gcf,'Tag','MessageLine'),'String',msg);
         return;
      end
      EditConditions;
...
function init
   ...
   if ~isempty(struct_session_profile_pos) & strcmp(save_setting_status,'on')

      pos = struct_session_profile_pos;
   ...
   h0 = figure('Color',[0.8 0.8 0.8], ...
   	'Units','normal', ...
        'Name','New Structural Session Information', ...
        'NumberTitle','off', ...
        'Menubar', 'none', ...
   	'Position', pos, ...
        'DeleteFcn','struct_session_profile_ui(''DELETE_FIGURE'');', ...
   ...
   c = uicontrol('Parent',h0, ...		% Session Description Edit
   	...
   	'Position',pos, ...
   	'String','', ...
   	'Callback','struct_session_profile_ui(''EDIT_DESCRIPTION'');', ...
   ...
   c = uicontrol('Parent',h0, ...		% PLS Data Directory Edit
   	...
   	'Callback','struct_session_profile_ui(''EDIT_PLS_DATA_DIR'');', ...
	'visible','off', ...
   ...
   c = uicontrol('Parent',h0, ...		% PLS Data Directory Button
   	...
   	'FontSize',fnt, ...
        ...
        'String','Browse ...', ...
        'Enable','On', ...
   	'Callback','struct_session_profile_ui(''SELECT_PLS_DATA_DIR'');', ...
	'visible','off', ...
   ...
   c = uicontrol('Parent',h0, ...		% Datamat Prefix Edit
   	...
   	'Position',pos, ...
   	'String','', ...
   	'Callback','struct_session_profile_ui(''EDIT_DATAMAT_PREFIX'');', ...
   ...
   c = uicontrol('Parent',h0, ...		% Number of Behavior Edit
   	...
   	'Position',pos, ...
   	'String','0', ...
   	'ButtonDownFcn','struct_session_profile_ui(''EDIT_NUM_BEHAVIOR'');', ...
	'enable', 'inactive', ...
	'visible', 'off', ...
   ...
   c = uicontrol('Parent',h0, ...		% Behav Data Edit
	...
	'string', 'Edit Behavior Data...', ...
   	...
   	'FontSize',fnt, ...
   	'Position',pos, ...
	'callback','struct_session_profile_ui(''EDIT_BEHAV_DATA'');', ...
	'enable', 'on', ...
	'visible', 'off', ...
   	'Tag','BehavDataEdit');
...
function LoadSessionInfo()
   ...
   [filename, pathname] = rri_selectfile( '*_STRUCTsessiondata.mat', 'Load a PLS session file');
...
%   setappdata(gcf,'SessionBehavDataFile',session_info.behav_data_file);
   ...
   if isempty(session_file)
      set(gcf,'Name','New Structural Session Information');
   else
...
if (status ~= 0),  struct_create_datamat({session_info});  end;
return;
...
function ModifyDatamat()
   ...
   [datamat_file, datamat_path] =  ...
	rri_selectfile('*_STRUCTsessiondata.mat', 'Open Structural Datamat');
