...
%
%------------------------------------------------------------------------
function fig = pet_plot_rf_task(action,varargin)
%
% pet_plot_rf_task(action,action_arg1,...)
...
if 0
   ...
   if ~isempty(pet_plot_rf_task_pos) & strcmp(save_setting_status,'on')

      pos = pet_plot_rf_task_pos;
   ...
   hh = figure('Units','normal', ...
	   ...
	   'Color', [0.8 0.8 0.8], ...
 	   'DoubleBuffer','on', ...
	   'DeleteFcn', 'pet_plot_rf_task(''DeleteFigure'');', ...
	   'Position',pos, ...
	   'Tag','PlotRFFig');
   ...
   h0 = uicontrol('Parent',hh, ...
	   ...
	   'tooltipstring', 'Use average intensity of its neighborhood voxels for intensity of this voxel. Neighborhood size is number of voxels from this voxel.', ...
           'CallBack', 'pet_plot_rf_task(''PlotBnPress'');', ...
   ...
   cb_fn = [ 'pet_plot_rf_task(''LoadSTDatamat'');'];
   ...
   h0 = uicontrol('Parent',hh, ...
	  ...
	  'BackgroundColor', [0.8 0.8 0.8], ...
          'String', 'Residualized', ...
          'CallBack', 'pet_plot_rf_task(''residualized'');', ...
	  'Tag', 'residualized');
   ...
   h2 = uimenu('parent',hh, ...
        'userdata', 1, ...
        'callback','pet_plot_rf_task(''zoom'');', ...
        'label','&Zoom on');
  ...
  h0 = uimenu('Parent',h_plot, ...
	     ...
	     'Callback','pet_plot_rf_task(''MENU_PlotIndividualData'');');
  h0 = uimenu('Parent',h_plot, ...
	     'Label','Plot group data', ...
	     'Tag','PlotGroupData', ...
		'visible', 'off', ...
	     'Callback','pet_plot_rf_task(''MENU_PlotGroupData'');');
  h0 = uimenu('Parent',h_plot, ...
	     'Label','Plot all data', ...
	     'Tag','PlotAllData', ...
	     'Callback','pet_plot_rf_task(''MENU_PlotAllData'');');
  ...
  h0 = uimenu('Parent',h_option, ...
	     'Label','Export data', ...
	     'Tag','ExportData', ...
	     'Callback','pet_plot_rf_task(''MENU_ExportData'');');
  h0 = uimenu('Parent',h_option, ...
	     'Label','Export data for behav analysis', ...
	     'Tag','ExportBehavData', ...
		'visible', 'off', ...
	     'Callback','pet_plot_rf_task(''MENU_ExportBehavData'');');
  ...
  h0 = uimenu('Parent',h_option, ...
	     'Label','Export data', ...
	     'Tag','ExportDataBehav', ...
	     'Callback','pet_plot_rf_task(''MENU_ExportDataBehav'');');
...
%           'Style', 'check', ...
  ...
  h1 = uicontrol('Parent',gcf, ...
   	   ...
   	   'Units','normal', ...
	   'Tag', 'CondSlider', ...
	   'Callback','pet_plot_rf_task(''SliderMotion'');');
...
function h = delete_fig()
  ...
  try
     ...
     pet_plot_rf_task_pos = get(gcbf,'position');

     save(pls_profile, '-append', 'pet_plot_rf_task_pos');
  catch
  end
...
%   data = mean(st_data(:, ncoord_idx), 2);
 %  data_mean = mean(reshape(data,[num_subj num_cond]),1)';
   ...
   if ischar(filename) & ~all(ismember('_pet_rf_plot',lower(filename)))
...
function  save_response_behav()
   ...
   if ischar(filename) & isempty(findstr(lower(filename),'_pet_datamatcorr_plot'))
      [tmp filename] = fileparts(filename);
...
function  plot_response_fn_resid()
   ...
   if isempty(st_datamat)
      ...
      for i=1:length(st_files)
         ...
         load(sessionfile,'datamat','coords','session_info');
         warning on;
