%
%   Usage: fig = pet_plot_taskpls_bs(LINK, fname);
%   Called by pet_result_ui
%
function taskpls_fig = pet_plot_taskpls_bs(varargin)
   ...
   switch (action),
     ...
     case {'zoom'}
         ...
         if (zoom_on_state == 1)                   % zoom on
            ...
            zoom off;
            set(gcf,'buttondown','pet_plot_taskpls_bs(''fig_bt_dn'');');
...
%      grp_str = [grp_str, {['Group ', num2str(i)]}];
   ...
   if ~isempty(pet_plot_taskpls_bs_pos) & strcmp(save_setting_status,'on')

      pos = pet_plot_taskpls_bs_pos;
   ...
   h0 = figure('units','normal', ...
	...
	'name',tit, ...
	'color', [0.8 0.8 0.8], ...
	'deleteFcn', 'pet_plot_taskpls_bs(''delete_fig'');', ...
	'doubleBuffer','on', ...
	'position',pos);
   ...
   hm_axis = uimenu('parent',h_view, ...
	'userdata', 0, ...
	'callback', 'pet_plot_taskpls_bs(''toggle_axis'');', ...
		'visible', 'off', ...
	'label','&Plot Current Figure');

   hm_dlv = uimenu('parent',h_view, ...
	'userdata', 0, ...
	'callback', 'pet_plot_taskpls_bs(''toggle_dlv'');', ...
		'visible', 'off', ...
...
%	'sepa','on', ...
   ...
   if(is_design_plot)
	...
	'callback', 'pet_plot_taskpls_bs(''ToggleBrainDesignScores'');', ...
	'tag','BrainDesignMenu');
      h1 = uimenu('parent',h_view, ...
	'label','Show &Design Scores', ...
	'userdata',0, ...		% show design scores
	'callBack', 'pet_plot_taskpls_bs(''ToggleDesignScores'');', ...
	'tag','DesignMenu');
      h1 = uimenu('parent',h_view, ...
	'label','Show Design &Latent Variables', ...
	'userdata',0, ...		% show design lv
	'callBack', 'pet_plot_taskpls_bs(''ToggleDesignLV'');', ...
	'tag','DesignLVMenu');
   else
      h1 = uimenu('parent',h_view, ...
	'sepa','on', ...
   ...
   h1 = uimenu('parent',h_view, ...
	...
	'callBack', 'pet_plot_taskpls_bs(''ToggleLegend'');', ...
	'tag','LegendMenu');
   ...
   h2 = uimenu('parent',h0, ...
	'visible', 'off', ...
        'userdata', 1, ...
        'callback','pet_plot_taskpls_bs(''zoom'');', ...
        'label','&Zoom on');
   ...
   lv_h = uicontrol('parent',h0, ...
	...
	'fontunit', 'normal', ...
	'fontsize', 0.5, ...
	'callback', 'pet_plot_taskpls_bs(''select_lv'');', ...
	'tag','LVRadioButton', ...
	'visible','off', ...
	'position', pos);
   ...
   h1 = uicontrol('parent',h0, ...
	...
	'min', 0, ...
	'max', 1, ...
	'callback', 'pet_plot_taskpls_bs(''move_slider'');', ...
	'tag','LVButtonSlider', ...
	'visible','off', ...
	'position', pos);
...
function PlotBrainDesignScores
   ...
   set(ax_hdl, ...
	'buttondown','pet_plot_taskpls_bs(''fig_bt_dn'');', ...
 ...
 for grp_idx = 1:num_grp
   ...
   for n=1:num_subjs
      for k=1:num_conds
		...
		color_code(k,:), ...
		'buttondown','pet_plot_taskpls_bs(''select_subj'');', ...
		'userdata', [grp_idx, n]);
...
function delete_fig(),
   ...
   try
     ...
     pet_plot_taskpls_bs_pos = get(gcbf,'position');

     save(pls_profile, '-append', 'pet_plot_taskpls_bs_pos');
   catch
   end
