function [cluster_info, cluster_fig] = struct_cluster_report(varargin)
%
% USAGE:  cluster_info =  struct_cluster_report(min_size, min_dist, ...
...
%   set(gcf,'Pointer','watch');
   ...
   [tmp datamat_file_lst{1}] = rri_fileparts(datamat_file_lst{1});
   datamat_file = load(datamat_file_lst{1},'coords','datafile','session_info');
   [tmp datamat_file.datafile] = rri_fileparts(datamat_file.datafile);
   load(datamat_file.datafile);
   datamat_file.datamat = datamat;
   clear datamat;
...
function  save_cluster_report()
   ...
   [filename, pathname] = ...
           rri_selectfile('*_STRUCTcluster.mat','Save to .mat file');
...
function  save_cluster_txt()
   ...
   [filename, pathname] = ...
           rri_selectfile('*_STRUCTcluster.txt','Save to .txt file');
...
function  save_peak_location()
   ...
   [filename, pathname] = ...
           rri_selectfile('*_STRUCTcluster_peak.txt','Export Peak Location');
...
function  save_all_location()
   ...
   [filename, pathname] = ...
           rri_selectfile('*_STRUCTcluster_all.txt','Export All Location');
...
function  cluster_fig = load_cluster_report(mainfig)
   ...
   [filename, pathname] = ...
           rri_selectfile('*_STRUCTcluster.mat','Load the cluster report');
