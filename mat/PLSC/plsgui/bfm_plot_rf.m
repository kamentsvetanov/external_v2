...
%
% apr 13,2005 jimmy: above change does not apply to bfm_plot_rf, because
%	st_datamat here only contains common_coords. Also see 
%	load_voxel_intensity to see how st_datamat get only common_coords. 
%
%------------------------------------------------------------------------
function fig = bfm_plot_rf(action,varargin)
%
% bfm_plot_rf(action,action_arg1,...)
...
if 0
   ...
   if ~isempty(bfm_plot_rf_pos) & strcmp(save_setting_status,'on')
   ...
   hh = figure('Units','normal', ...
	   ...
	   'Color', [0.8 0.8 0.8], ...
 	   'DoubleBuffer','on', ...
	   'DeleteFcn', 'bfm_plot_rf(''DeleteFigure'');', ...
	   'Position',pos, ...
	   'Tag','PlotRFFig');
   ...
   cb_fn = [ 'bfm_plot_rf(''change_group'');'];
   ...
   h0 = uicontrol('Parent',hh, ...
	  ...
	  'visible', 'off', ...
	  'Tag', 'PlotButton', ...
          'CallBack', 'bfm_plot_rf(''PlotBnPress'');');
  ...
  h0 = uimenu('Parent',h_plot, ...
	     ...
	     'Callback','bfm_plot_rf(''MENU_PlotIndividualData'');');
  h0 = uimenu('Parent',h_plot, ...
	     'Label','Plot group data', ...
	     'Tag','PlotGroupData', ...
		'visible', 'off', ...
	     'Callback','bfm_plot_rf(''MENU_PlotGroupData'');');
  h0 = uimenu('Parent',h_plot, ...
	     'Label','Plot all data', ...
	     'Tag','PlotAllData', ...
	     'Callback','bfm_plot_rf(''MENU_PlotAllData'');');
  ...
  h0 = uimenu('Parent',h_option, ...
	     'Label','Export data', ...
	     'Tag','ExportData', ...
	     'Callback','bfm_plot_rf(''MENU_ExportData'');');
  h0 = uimenu('Parent',h_option, ...
	     'Label','Export data for behav analysis', ...
	     'Tag','ExportBehavData', ...
		'visible', 'off', ...
	     'Callback','bfm_plot_rf(''MENU_ExportBehavData'');');
  ...
  h0 = uimenu('Parent',h_option, ...
	     'Label','Export data', ...
	     'Tag','ExportDataBehav', ...
	     'Callback','bfm_plot_rf(''MENU_ExportDataBehav'');');
