function [blv_img,cmap,cbar_map] = fmri_plot_brainlv(brainlv,coords,dims, ...
	slice_idx,thresh,thresh2,range,rot_amount,bg_img,cluster_idx)
%
%  USAGE: [blv_img,cmap,cbar_map] = fmri_plot_brainlv(brainlv,coords,dims, ...
%	slices_idx,thresh,range,rot_amount,bg_img,cluster_idx)
%	
%  INPUT: 
%     brainlv - a row vector contains one of the brainlvs to be shown 
%     coords - the coords for the brain voxels
%     dims -  a vector, [rows cols 1 slices], specifies the dimension
%             of the brain volume.
...
%     rot_amount - (optional) number of times to rotate the image with 90 degree
%     bg_img - (optional) background image
%     cluster_idx - cluster_info.data{i}.idx in most recent cluster report
%                           
%  OUTPUT: 
%     blv_img - the 4D matrix for the brainlv
%     cmap - colormap to be used to display the images
%     cbar_map - a colormap for displaying the colorbar
%
%  Example:
%
%      dims = [64 64 1 124];
%      slice_idx = [50:70];
%      range = [-0.8 0.8];
%      [blv_img,cmap] = plot_brainlv(brainlv,coords,dims,slice_idx,0.4,range,1);
%
%   Created Aug 2000 by Wilkin Chau, Rotman Research Institute
