...
% 'floor' chnaged to 'round' on line 318 336 338
...
function fmri_get_datamat(sessionFile, run_idx, varargin)
   ...
   elseif (nargin >= 2),
      ...
      if (nargin >= 16)
         SingleRefScanButton = varargin{14};
      end;

      if (nargin >= 17)
         SingleRefScanOnset = varargin{15};
      end;

      if (nargin >= 18)
         SingleRefScanNumber = varargin{16};
      end;
   ...
   for i = run_idx
      ...
      for j = 1:num_conds
         ...
         for k = 1:num_onsets
            ...
            start_scan = round(run_info.evt_onsets{j}(k) - num_skipped_scans) + 1;
            ...
            elseif (baseline_start <= 0)
                disp(sprintf('Scans %03d for the condition %d of run%d are not included due to out of bound baseline', round(run_info.evt_onsets{j}(k)), j, run_idx(i)));
            else
...
if 0
   ...
   big_coords = win_size*coords;
   big_coords = big_coords(:)';
   big_coords2 = big_coords;

   for i = 1:win_size - 1
      big_coords2 = big_coords2 - 1;
      big_coords = [big_coords2; big_coords];
   end
end

   st_datamat = datamat(:,big_coords(:));
...
%   st_datamat = st_datamat(order_idx,:);
 %  st_evt_list = new_evt_list;
   ...
   fname = sprintf('%s_fMRIsessiondata.mat',datamat_prefix);
...
if ~for_batch
   ...
   l_coords = length(st_coords);
   st_datamat = reshape(st_datamat, [r_mat*st_win_size l_coords]);
