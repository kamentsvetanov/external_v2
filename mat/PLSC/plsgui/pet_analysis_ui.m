   ...
   switch (action)
     ...
     case {'MENU_CREATE_CONTRASTS'}
        ...
        if get(findobj(gcf,'Tag','SelectNonRotateMultiblock'),'value')
           designdata = rri_input_contrast_ui({'PET'},pls_session,cond_selection,num_groups,'nonrotatemultiblock',0,behavname,ContrastMatrix);
        else
           designdata = rri_input_contrast_ui({'PET'},pls_session,cond_selection,num_groups,[],0,behavname,ContrastMatrix);
        end
	...
	SelectPosthocDataFile;
     case {'BROWSE_CONTRAST_FILE'}
        ...
        [contrast_file,contrast_path] = rri_selectfile('*_PETcontrast.txt',dlg_title);
        if (contrast_file ~= 0)
...
function init()
   ...
   h1 = uicontrol('Parent',h0, ...
	...
	'tag','SelectMultiblockData', ...
   	'Callback','pet_analysis_ui(''MULTIBLOCK_SELECTED'');', ...
	'enable','on', ...
	'position',pos);
   ...
   h1 = uicontrol('Parent',h0, ...
	...
	'tag','SelectNonRotateBehav', ...
   	'Callback','pet_analysis_ui(''NON_ROTATE_BEHAVPLS_SELECTED'');', ...
	'position',pos);
   ...
   h1 = uicontrol('Parent',h0, ...
	...
	'tag','SelectNonRotateMultiblock', ...
   	'Callback','pet_analysis_ui(''NON_ROTATE_MULTIBLOCK_SELECTED'');', ...
	'position',pos);
   ...
   h1 = uicontrol('Parent',h0, ...
	...
	'fontunit','normal', ...
   	...
   	'String','0', ...
   	'Style','edit', ...
   	'Callback','pet_analysis_ui(''EDIT_NUM_PERM'');', ...
   ...
   h1 = uicontrol('Parent',h0, ...
	...
	'fontunit','normal', ...
   	...
   	'Style','edit', ...
	'Enable', 'on', ...
   	'Callback','pet_analysis_ui(''EDIT_NUM_SPLIT'');', ...
   	'Tag','NumSplitEdit');
   ...
   h1 = uicontrol('Parent',h0, ...
	...
	'visible', 'off', ...
   	...
   	'Callback','pet_analysis_ui(''NONROTATED_BOOT'');', ...
        'Tag','nonrotated_boot');
...
%   x = 0.5 - w/2;
   ...
   h1 = uicontrol('Parent',h0, ...
	...
	'fontunit','normal', ...
   	...
   	'Callback','pet_analysis_ui(''CRUN_BUTTON_PRESSED'');', ...
   	'Tag','CRUNButton');
   ...
   h1 = uicontrol('Parent',h0, ...
	...
	'fontunit','normal', ...
   	...
   	'Callback','pet_analysis_ui(''LOADBATCH_BUTTON_PRESSED'');', ...
   	'Tag','LOADBATCHButton');
 ...
 %       'Label', 'Modify Behavior Data (behavior data should only contain the above selected conditions)', ...
  % 	'Callback','pet_analysis_ui(''MENU_MODIFY_BEHAVDATA'');', ...
   ...
   m1 = uimenu(h_deselect, ...
   	...
   	'Callback','pet_analysis_ui(''MENU_MULTIBLOCK_DESELECT_CONDITION'');', ...
	'enable', 'off', ...
...
function AddGroupProfiles()
   ...
   if isempty(group_profile)
      ...
      pf_path = fullfile(pf_path, '*_PETsessiondata.mat');
...
function ExecutePLS()
   ...
   if exist('plslog.m','file')
      if PLSoptions.ismultiblock
         plslog('PET Multiblock Analysis');
      elseif PLSoptions.isnonrotatebehav
         plslog('PET Non-Rotated Behavior Analysis');
      elseif PLSoptions.isnonrotatemultiblock
         plslog('PET Non-Rotated Multiblock Analysis');
      elseif PLSoptions.isbehav
         plslog('PET Regular Behavior Analysis');
      elseif PLSoptions.iscontrast
         plslog('PET Non-Rotated Task Analysis');
      else
         plslog('PET Mean-Centering Analysis');
      end
   end

   [resultFile] = pet_analysis(PLSoptions.isbehav, ...
...
function status = cExecutePLS()
   ...
   if exist('plslog.m','file')
      if PLSoptions.ismultiblock
         plslog('cPET Multiblock Analysis');
      elseif PLSoptions.isnonrotatebehav
         plslog('cPET Non-Rotated Behavior Analysis');
      elseif PLSoptions.isnonrotatemultiblock
         plslog('cPET Non-Rotated Multiblock Analysis');
      elseif PLSoptions.isbehav
         plslog('cPET Regular Behavior Analysis');
      elseif PLSoptions.iscontrast
         plslog('cPET Non-Rotated Task Analysis');
      else
         plslog('cPET Mean-Centering Analysis');
      end
   end
   ...
   [result_file,result_path] = ...
      rri_selectfile([datamat_prefix,'_PETresult.mat'],'Please enter a result file name that will be used by the created batch file');
   ...
   [input_file,input_path] = ...
      rri_selectfile([datamat_prefix,'_PETanalysis.txt'],'Please enter a batch file name that will be used by "batch_plsgui"');
...
%   progress_hdl = rri_progress_status('create','PLS Analysis');
   ...
   if isempty(selected_cond) | sum(selected_cond) == 0 | ...
	( length(selected_cond) ~= session_info.num_conditions & ...
          isempty(findstr(first_file, '_PETsessiondata.mat')) )
...
%   end
   ...
   if ~isempty(findstr(first_file, '_PETsessiondata.mat'))	% PET
