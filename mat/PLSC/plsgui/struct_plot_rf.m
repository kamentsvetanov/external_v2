...
if 0
   ...
   hh = figure('Units','normal', ...
	   ...
	   'Color', [0.8 0.8 0.8], ...
 	   'DoubleBuffer','on', ...
	   'DeleteFcn', 'struct_plot_rf(''DeleteFigure'');', ...
	   'Position',pos, ...
	   'Tag','PlotRFFig');
   ...
   cb_fn = [ 'struct_plot_rf(''LoadSTDatamat'');'];
   ...
   h0 = uicontrol('Parent',hh, ...
	  ...
	  'visible', 'off', ...
	  'Tag', 'PlotButton', ...
          'CallBack', 'struct_plot_rf(''PlotBnPress'');');
  ...
  h0 = uimenu('Parent',h_option, ...
	     'Label','Export data', ...
	     'Tag','ExportDataBehav', ...
	     'Callback','struct_plot_rf(''MENU_ExportDataBehav'');');
...
%           'Style', 'check', ...
  ...
  h1 = uicontrol('Parent',gcf, ...
   	   ...
   	   'Units','normal', ...
	   'Tag', 'CondSlider', ...
	   'Callback','struct_plot_rf(''SliderMotion'');');
...
function h = delete_fig()
  ...
  try
     ...
     struct_plot_rf_pos = get(gcbf,'position');

     save(pls_profile, '-append', 'struct_plot_rf_pos');
  catch
  end
...
%   coord_idx = find(org_coords == cur_coord);
   ...
   if ischar(filename) & ~all(ismember('_struct_datamatcorr_plot',lower(filename)))
      [tmp filename] = fileparts(filename);
