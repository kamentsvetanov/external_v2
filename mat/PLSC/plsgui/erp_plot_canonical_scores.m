...
function h = delete_fig()
  ...
  try
     ...
     erp_plot_canonical_scores_pos = get(gcbf,'position');

     save(pls_profile, '-append', 'erp_plot_canonical_scores_pos');
  catch
  end

  main_fig = getappdata(gcbf,'main_fig');
  hm_canonical = getappdata(main_fig,'hm_canonical');
...
function get_st_datamat_filename(plsResultFile)
  ...
  load(plsResultFile,'datamat_files');
...
%--------------------------------------------------------------------------
%
function  [st_datamat,st_win_size,st_evt_list] = load_plotted_datamat(selected_idx)
...
%---------------------------------------------------------------------------
function [brainlv,conditions,cond_selection,chan_selection,common_time_info] = ...
							load_pls_brainlv(fname)
...
if 0
   try 
      load( fname,'salience','cond_name','common_conditions','common_channels','common_time_info');
   catch
...
function  status = get_brain_scores()
   ...
   if (strcmp(last_datamat,selected_data) == 0)
       ...
       [st_datamat, st_win_size, st_evt_list] = load_plotted_datamat(selected_idx);
...
function  save_response_fn()
   ...
   [filename, pathname] = ...
	rri_selectfile('*_ERP_bs_plot.mat','Save the Temporal Scores');

   if ischar(filename) & ~all(ismember('_erp_bs_plot',lower(filename)))
      [tmp filename] = fileparts(filename);
      filename = [filename, '_ERP_bs_plot.mat'];
   end
   ...
   try
     save (rf_plot_file, 'selected_files', 'bs_data', 'st_evt_list', 'select_lv');
   catch
