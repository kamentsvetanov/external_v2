...
%      set(gcf,'pointer','arrow');
  ...
  elseif strcmp(action,'zoom')
         ...
         if (zoom_on_state == 1)                   % zoom on
            ...
            zoom off;
            set(gcf,'buttondown','ssb_bfm_plot_rf_task(''fig_bt_dn'');');
...
if 0
   ...
   hh = figure('Units','normal', ...
	   ...
	   'Color', [0.8 0.8 0.8], ...
 	   'DoubleBuffer','on', ...
	   'DeleteFcn', 'ssb_bfm_plot_rf_task(''DeleteFigure'');', ...
	   'Position',pos, ...
	   'Tag','PlotRFFig');
   ...
   h0 = uicontrol('Parent',hh, ...
	   ...
	   'tooltipstring', 'Use average intensity of its neighborhood voxels for intensity of this voxel. Neighborhood size is number of voxels from this voxel.', ...
           'CallBack', 'ssb_bfm_plot_rf_task(''PlotBnPress'');', ...
   ...
   cb_fn = [ 'ssb_bfm_plot_rf_task(''change_group'');'];
   ...
   h0 = uicontrol('Parent',hh, ...
	  ...
	  'BackgroundColor', [0.8 0.8 0.8], ...
          'String', 'Residualized', ...
          'CallBack', 'ssb_bfm_plot_rf_task(''residualized'');', ...
	  'Tag', 'residualized');
   ...
   h2 = uimenu('parent',hh, ...
        'userdata', 1, ...
        'callback','ssb_bfm_plot_rf_task(''zoom'');', ...
        'label','&Zoom on');
  ...
  h0 = uimenu('Parent',h_plot, ...
	     ...
	     'Callback','ssb_bfm_plot_rf_task(''MENU_PlotIndividualData'');');
  h0 = uimenu('Parent',h_plot, ...
	     'Label','Plot group data', ...
	     'Tag','PlotGroupData', ...
		'visible', 'off', ...
	     'Callback','ssb_bfm_plot_rf_task(''MENU_PlotGroupData'');');
  h0 = uimenu('Parent',h_plot, ...
	     'Label','Plot all data', ...
	     'Tag','PlotAllData', ...
	     'Callback','ssb_bfm_plot_rf_task(''MENU_PlotAllData'');');
  ...
  h0 = uimenu('Parent',h_option, ...
	     'Label','Export data', ...
	     'Tag','ExportData', ...
	     'Callback','ssb_bfm_plot_rf_task(''MENU_ExportData'');');
  h0 = uimenu('Parent',h_option, ...
	     'Label','Export data for behav analysis', ...
	     'Tag','ExportBehavData', ...
		'visible', 'off', ...
	     'Callback','ssb_bfm_plot_rf_task(''MENU_ExportBehavData'');');
  ...
  h0 = uimenu('Parent',h_option, ...
	     'Label','Export data', ...
	     'Tag','ExportDataBehav', ...
	     'Callback','ssb_bfm_plot_rf_task(''MENU_ExportDataBehav'');');
...
%	   'Callback',cbf);
  ...
  h1 = uicontrol('Parent',gcf, ...
   	   ...
   	   'Units','normal', ...
	   'Tag', 'CondSlider', ...
	   'Callback','ssb_bfm_plot_rf_task(''SliderMotion'');');
...
function  plot_response_fn()
   ...
   set(gca,'buttondown','ssb_bfm_plot_rf_task(''fig_bt_dn'');');
...
%   coord_idx = find(st_coords == cur_coord);
   ...
   for k = cond_idx			% cond
      num_subj = length(subj_name{k});

      for n = 1:num_subj		% subj
         j = n + last_num_subj;		% row number in datamat
         intensity(k,n) = mean(st_data(j, ncoord_idx), 2);
         intensity2 = [intensity2; intensity(k,n)];
         intensity_hdl(k,n) = 0;	% initialization
      end

      intensity_avg(k) = mean(intensity(k,[1:num_subj]),2);
      intensity_avg_hdl(k) = 0;

      last_num_subj = last_num_subj + num_subj;
   end

   data_mean = intensity_avg(:);

   setappdata(gcf,'intensity',intensity2);
   setappdata(gcf,'intensity_avg',data_mean);
   ...
   % need more color
   %
   if length([subj_name{:}]) > size(color_code,1)

      tmp = [];

      for i=1:ceil( length([subj_name{:}])/size(color_code,1) )
         tmp = [tmp; color_code];
      end
   ...
   for k = cond_idx
      ...
      for n = 1:num_subj
         intensity_hdl(k,n) = plot(k,intensity(k,n), ...
			color_code(n,:), ...
			'userdata', [n k], ...
			'buttondown', 'ssb_bfm_plot_rf_task(''select_subj'');');
      end
   ...
   setappdata(gcf, 'intensity_hdl', intensity_hdl);
   setappdata(gcf, 'data_mean', data_mean);
...
%   coord_idx = find(st_coords == cur_coord);
   ...
   subj_name = getappdata(gcf,'subj_name');
   subj_name = subj_name{selected_idx};
   num_subj = cellfun('length',subj_name);

   st_evt_list = [];

   for i=1:num_cond
      st_evt_list = [st_evt_list i*ones(1, num_subj(i))];
   end
...
uiwait(msgbox('Need use new version to run PLS analysis again in order to get the correlation file'));
   ...
   if iscell(subj_group)
      ...
      for i = 1:num_group
         first = last + 1;
         last = last + subj_group(i);

         cond_subj_name = {};

         for j = 1:num_conditions
            cond_subj_name = [cond_subj_name {subj_name(first:last)}];
         end

         org_subj_name = [org_subj_name {cond_subj_name}];
      end
   end
...
%         DatamatFile{i}{n} = strrep(SessionProfiles{i}{n}, 'session', 'datamat');
		...
		[session_info.datamat_prefix, '_BfMRIdatamat.mat']);
         ...
         for k = find(cond_selection)
            mask1 = [mask1 find(st_evt_list == k)];
         end

         st_evt_list = st_evt_list(mask1);

         [tmp, idx] = intersect(st_coords, common_coords);
         st_datamat = st_datamat(mask1,idx);
...
%         subj_name = [subj_name, {['Subj', num2str(n)]}];
         subj_name = [subj_name, org_subj_name];

         mask = [mask st_evt_list];

      end					% for n = 1:num_subj
...
if 0
   ...
   setappdata(gcf,'subj_name',org_subj_name);
...
function select_subj
   ...
   intensity_hdl = getappdata(gcf,'intensity_hdl');
   ...
   for i=1:length(intensity_hdl(:))
      set(intensity_hdl(i), 'selected', 'off');		% remove selection
   end
