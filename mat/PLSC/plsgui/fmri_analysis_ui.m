function fig = fmri_analysis_ui(varargin) 
   ...
   switch (action)
     ...
     case {'MENU_CREATE_CONTRASTS'}
        ...
        if get(findobj(gcf,'Tag','SelectNonRotateMultiblock'),'value')
           designdata = rri_input_contrast_ui({'fMRI'},pls_session,cond_selection,num_groups,'nonrotatemultiblock',0,behavname,ContrastMatrix);
        else
           designdata = rri_input_contrast_ui({'fMRI'},pls_session,cond_selection,num_groups,[],0,behavname,ContrastMatrix);
        end
	...
	SelectContrastData;
     case {'BROWSE_CONTRAST_FILE'}
        ...
        [contrast_file,contrast_path] = rri_selectfile('*_fMRIcontrast.txt',dlg_title);
        if (contrast_file ~= 0)
...
%              uiresume(gcf);
     ...
     case {'DELETE_FIGURE'}
        ...
        try
           ...
           fmri_analysis_pos = get(gcbf,'position');

           save(pls_profile, '-append', 'fmri_analysis_pos');
        catch
        end
     otherwise
...
function Init()
   ...
   if ~isempty(fmri_analysis_pos) & strcmp(save_setting_status,'on')

      pos = fmri_analysis_pos;
   ...
   h0 = figure('Color',[0.8 0.8 0.8], ...
	'Units','normal', ...
	'Name','PLS Analysis for Event Related fMRI sessions', ...
	'NumberTitle', 'off', ...
   	'Position',pos, ...
   	'Menubar','none', ...
        'DeleteFcn','fmri_analysis_ui(''DELETE_FIGURE'');', ...
   ...
   h1 = uicontrol('Parent',h0, ...		% scroll bar
	'Style', 'slider', ...
   	...
   	'Value',20, ...
   	'Position',pos, ...
   	'Callback','fmri_analysis_ui(''MOVE_SLIDER'');', ...
   	'Tag','GroupSlider');
   ...
   h1 = uicontrol('Parent',h0, ...
	...
	'visible','off', ...
   	'Callback','fmri_analysis_ui(''GROUP_ANALYSIS_SELECTED'');', ...
   ...
   h1 = uicontrol('Parent',h0, ...
	...
	'Value',1, ...
	'tag','SelectMean', ...
   	'Callback','fmri_analysis_ui(''CONTRAST_NONE_SELECTED'');', ...
	'position',pos);
   ...
   h1 = uicontrol('Parent',h0, ...
	...
	'tag','SelectNonRotateBehav', ...
   	'Callback','fmri_analysis_ui(''NON_ROTATE_BEHAVPLS_SELECTED'');', ...
	'position',pos);
   ...
   h1 = uicontrol('Parent',h0, ...
	...
	'tag','SelectNonRotateMultiblock', ...
   	'Callback','fmri_analysis_ui(''NON_ROTATE_MULTIBLOCK_SELECTED'');', ...
	'position',pos);
   ...
   h1 = uicontrol('Parent',h0, ...
	...
	'fontunit','normal', ...
   	...
   	'String','0', ...
   	'Style','edit', ...
   	'Callback','fmri_analysis_ui(''EDIT_NUM_PERM'');', ...
   ...
   h1 = uicontrol('Parent',h0, ...
	...
	'fontunit','normal', ...
   	...
   	'Style','edit', ...
	'Enable', 'on', ...
   	'Callback','fmri_analysis_ui(''EDIT_NUM_SPLIT'');', ...
   	'Tag','NumSplitEdit');
   ...
   h1 = uicontrol('Parent',h0, ...
	...
	'visible', 'off', ...
   	...
   	'Callback','fmri_analysis_ui(''NONROTATED_BOOT'');', ...
        'Tag','nonrotated_boot');
   ...
   h1 = uicontrol('Parent',h0, ...
	...
	'fontunit','normal', ...
   	...
   	'Callback','fmri_analysis_ui(''LOADBATCH_BUTTON_PRESSED'');', ...
   	'Tag','LOADBATCHButton');
...
function SetupGroupRows()
   ...
   edit_cbf = '';
   delete_cbf = 'fmri_analysis_ui(''DELETE_GROUP'');';
   select_cbf = 'fmri_analysis_ui(''SELECT_GROUP'');';
...
function CreateAddRow()
   ...
   set(a_hdls(3),'String','Add','Visible','off', ...
		     'Callback','fmri_analysis_ui(''ADD_GROUP'');');
...
function status = SavePLSOptions
   ...
   for i=1:num_groups,
      ...
      for j=1:length(PLSoptions.profiles{i})
         ...
         tmp = [tmp.session_info.datamat_prefix '_fMRIsessiondata.mat'];
...
function total_profiles = ValidateProfiles(group_profiles,conditions),
   ...
   dm = load([s.session_info.datamat_prefix '_fMRIsessiondata.mat'], 'create_ver', 'SingleSubject');
 ...
 %  setappdata(progress_hdl,'ProgressScale',progress_merge_factor * 0.2);
  % setappdata(progress_hdl,'ProgressStart',0);
   ...
   if exist('plslog.m','file')
      ...
      case 2
         plslog('fMRI Non-Rotated Multiblock Analysis');
      case 3
         plslog('fMRI Non-Rotated Task Analysis');
      case 4
         plslog('fMRI Regular Behavior Analysis');
      case 5
         plslog('fMRI Multiblock Analysis');
      case 6
         plslog('fMRI Non-Rotated Behavior Analysis');
      end
   end
...
function status = cExecutePLS()
   ...
   if exist('plslog.m','file')
      switch (PLSoptions.ContrastMethod)
      case 1
         plslog('cfMRI Mean-Centering Analysis');
      case 2
         plslog('cfMRI Non-Rotated Multiblock Analysis');
      case 3
         plslog('cfMRI Non-Rotated Task Analysis');
      case 4
         plslog('cfMRI Regular Behavior Analysis');
      case 5
         plslog('cfMRI Multiblock Analysis');
      case 6
         plslog('cfMRI Non-Rotated Behavior Analysis');
      end
   end
...
%   progress_hdl = rri_progress_status('create','PLS Analysis');
   ...
   if isempty(selected_cond) | sum(selected_cond) == 0 | ...
	( length(selected_cond) ~= session_info.num_conditions & ...
          isempty(findstr(first_file, '_fMRIsessiondata.mat')) )
...
%   elseif ~isempty(findstr(first_file, '_fMRIsessiondata.mat'))	% E.R. fMRI
   ...
   if length(first_file)>18 & strcmpi(first_file(end-18:end),'fMRIsessiondata.mat')
      ...
      if length(first_file)>19 & strcmpi(first_file(end-19:end),'_fMRIsessiondata.mat')
      else
