...
%   set(fig_hdl, 'windowbuttonup', 'draw_edit1({''stopdragpt''})');

   axes(ax_hdl);
   line_hdl = plot(y);
set(ax_hdl,'xtick',[]);
%   set(line_hdl, 'buttondown', 'draw_edit1({''startdragpt''})');
set(ax_hdl, 'xlim', [1 length(y)]);
...
function startdragpt()
   ...
   set(gcf, 'windowbuttonmotion', 'draw_edit1({''dragpt''})');
