...
function DPARSF_OpeningFcn(hObject, eventdata, handles, varargin)
    ...
    if ~(exist('rest.m')&&exist('spm.m'))
        uiwait(msgbox('DPARSF is based on SPM and REST, Please install Matlab 7.3, SPM5, REST V1.8 or later version at first.','DPARSF'));
    else
...
function [handles, CheckingPass]=CheckCfgParameters(handles)    
    ...
    if (handles.Cfg.IsNeedConvertFunDCM2IMG==1)
        ...
        if (handles.Cfg.IsSliceTiming==1) || (handles.Cfg.IsRealign==1) || (handles.Cfg.IsNormalize>0)
            if 7==exist([handles.Cfg.WorkingDir,filesep,'FunImg'],'dir')
                if isempty (handles.Cfg.SubjectID)
                    Dir=dir([handles.Cfg.WorkingDir,filesep,'FunImg']);
                    if strcmpi(Dir(3).name,'.DS_Store')  %110908 YAN Chao-Gan, for MAC OS compatablie
                        StartIndex=4;
                    else
                        StartIndex=3;
                ...
                if handles.Cfg.IsSliceTiming==1
                    DirImg=dir([handles.Cfg.WorkingDir,filesep,'FunImg',filesep,handles.Cfg.SubjectID{1},filesep,'*.img']);
                end
                if (handles.Cfg.IsSliceTiming==0) && (handles.Cfg.IsRealign==1)
                    DirImg=dir([handles.Cfg.WorkingDir,filesep,'FunImg',filesep,handles.Cfg.SubjectID{1},filesep,'a*.img']);
                end
                if (handles.Cfg.IsSliceTiming==0) && (handles.Cfg.IsRealign==0) && (handles.Cfg.IsNormalize>0)
                    DirImg=dir([handles.Cfg.WorkingDir,filesep,'FunImg',filesep,handles.Cfg.SubjectID{1},filesep,'ra*.img']);
                end
            ...
            if (handles.Cfg.IsSmooth==1)
                if 7==exist([handles.Cfg.WorkingDir,filesep,'FunImgNormalized'],'dir')
                    if isempty (handles.Cfg.SubjectID)
                        Dir=dir([handles.Cfg.WorkingDir,filesep,'FunImgNormalized']);
                        if strcmpi(Dir(3).name,'.DS_Store')  %110908 YAN Chao-Gan, for MAC OS compatablie
                            StartIndex=4;
                        else
                            StartIndex=3;
                    ...
                    DirImg=dir([handles.Cfg.WorkingDir,filesep,'FunImgNormalized',filesep,handles.Cfg.SubjectID{1},filesep,'*.img']);
                ...
                if (handles.Cfg.IsDetrend==1) || (handles.Cfg.IsFilter==1)
                    ...
                    if (handles.Cfg.IsDetrend==0) && (handles.Cfg.IsFilter==1)
                        FunImgDir=[FunImgDir,'Detrended'];
                    end
                    if 7==exist([handles.Cfg.WorkingDir,filesep,FunImgDir],'dir')
                        if isempty (handles.Cfg.SubjectID)
                            Dir=dir([handles.Cfg.WorkingDir,filesep,FunImgDir]);
                            if strcmpi(Dir(3).name,'.DS_Store')  %110908 YAN Chao-Gan, for MAC OS compatablie
                                StartIndex=4;
                            else
                                StartIndex=3;
                    ...
                    if (handles.Cfg.IsCalReHo==1) || (handles.Cfg.IsCalALFF==1) || (handles.Cfg.IsCalfALFF==1)
                        ...
                        if (handles.Cfg.IsCalfALFF==0)
                            FunImgDir=[FunImgDir,'Filtered'];
                        end
                        if 7==exist([handles.Cfg.WorkingDir,filesep,FunImgDir],'dir')
                            if isempty (handles.Cfg.SubjectID)
                                Dir=dir([handles.Cfg.WorkingDir,filesep,FunImgDir]);
    ...
    if (handles.Cfg.IsSliceTiming==1) && (length(handles.Cfg.SliceTiming.SliceOrder)==length([1:2:25,2:2:24]))
        if all(handles.Cfg.SliceTiming.SliceOrder==[1:2:25,2:2:24])
    ...
    if (handles.Cfg.DataIsSmoothed==1) && (handles.Cfg.IsCalReHo==1)
