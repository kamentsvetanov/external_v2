function y_RegressOutImgCovariates(DataDir,CovariablesDef,Postfix,MaskFilename, ImgCovModel)
...
if ~isempty(CovariablesDef.CovImgDir)
    ...
    for i=1:length(CovariablesDef.CovImgDir);
        ...
        [CovTempVolume] =rest_to4d(CovariablesDef.CovImgDir{i});
...
if ~isempty(MaskFilename)
    [MaskData,MaskVox,MaskHead]=rest_readfile(MaskFilename);
else
