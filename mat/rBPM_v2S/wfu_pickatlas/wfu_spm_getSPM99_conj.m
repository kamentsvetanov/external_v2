function [SPM,VOL,xX,xCon,xSDM] = wfu_spm_getSPM99_conj

% This function combines the conjunction analysis of T. Nichols' toolbox
% (spm_getSPM.m) with the ROI analysis of J. Maldjian's WFU_PickAtlas 
% (wfu_spm_getSPM2.m). Conjunction-related code has been labelled with 
% multiple %'s
%----------------------------------------------------------------------
% computes a specified and thresholded following parameter estimation
...
%_______________________________________________________________________
% @(#)spm_getSPM.m	2.39 Andrew Holmes, Karl Friston & Jean-Baptiste Poline 01/09/07
...
end % if nc>1 & n>1... %%%%%%%formerly: if length(Ic)
...
if spm_input('ROI Analysis','+1','y/n',[1,0],2)
	...
	if ~isempty(wfu_atlas_filename)
		P = spm_get('Files',swd,'beta*.img');
                if isempty(P)
		    P = spm_get('Files',swd,'spmT*.img');   %get spmT
                end
                if isempty(P)
		    error('WFU_PickAtlas requires beta or spmT image for realignment');
                end
		P = {char(P(1,:)),wfu_atlas_filename};
		flags.mean=0;
		flags.hold = 0;
		flags.which=1;
		flags.mask=0;
...
if ~isempty(wfu_atlas_filename)
	...
	mXYZ  = D.mat \ [XYZmm; ones(1, size(XYZmm, 2))];
	j     = spm_sample_vol(D, mXYZ(1,:),mXYZ(2,:),mXYZ(3,:),0) > 0;	
	%str   = sprintf('image mask: %s',spm_str_manip(wfu_atlas_filename,'a30'));
	%-Compute in-mask volume S:
	% Correct for differences in mask and SPM voxel sizes
	VOX=sqrt(sum(M(1:3,1:3).^2))';
	Y     = spm_read_vols(D);
	vsc   = sqrt(sum(D.mat(1:3,1:3).^2)) ./ VOX';
	S     = sum(Y(:)>0) * prod(vsc);
	%Z     = Z(j);
	XYZ   = XYZ(:,j);
	XYZmm = XYZmm(:,j);
	QQ    = QQ(j);
	R     	= spm_resels(FWHM./vsc,D,SPACE);
	R 	=R;
	S  	= S;
	xSDM.R	= R;
	xSDM.S 	= S;
...
if ~isempty(XYZ)
	...
	u  = spm_uc_FDR(u,edf,STAT,n,VspmSv,mapped_mask);
    else
