function V = spm_create_vol2(V,varargin)
...
normalized = 0;
if isfield(V,'wfu')
	if isfield(V.wfu,'spm99norm'), spm99norm = V.wfu.spm99norm; end
	if isfield(V.wfu,'normalized'), normalized = V.wfu.normalized; end
end
if spm99norm & ~normalized,
	disp('spm99norm and normalized field mismatch');
	disp(V.fname);
	normalized = spm99norm;
end
if spm_flip_analyze_images & ~spm99norm, M = diag([-1 1 1 1])*M; end;
...
if spm_flip_analyze_images & ~spm99norm, mt = diag([-1 1 1 1])*mt; end;

if sum((V.mat(:) - mt(:)).*(V.mat(:) - mt(:))) > eps*eps*12 | exist(fname)==2,
	if exist(fname)==2,
		%-------------------------------------------------------
		%This code only executes if there is already a .mat, whether or not
		%there is a mismatch between mat and vx
		%For backward compatibility with SPM99, we are keeping M = mat for all
		...
		elseif isfield(str,'M'),
			%--------------------------------------------------------------------
			%This code only executes if the .mat was an spm99 file
			%it is concerned with correcting mat for the M in the spm99 matfile
			%--------------------------------------------------------------------
			mat = str.M;
			if spm_flip_analyze_images & ~spm99norm,
				for i=1:size(mat,3),
		...
		%------------------------------
		%This code is updating 4D mats
		%------------------------------
		mat(:,:,V.n) = V.mat;
		mat          = fill_empty(mat,mt);
		M = mat(:,:,1); if spm_flip_analyze_images & ~normalized, M = diag([-1 1 1 1])*M; end;
		try,
		...
		%-------------------------------------------------------
		%This code only executes if there is no .mat, but there
		%there is a mismatch between mat and vx
		%it is concerned with correcting M, not mat
		%-------------------------------------------------------
		clear mat
		mat(:,:,V.n) = V.mat;
		mat          = fill_empty(mat,mt);
...
if exist(fname)==0
	mat = V.mat;
	%------------------------------------------------------------------
	%routine should actually check if it is normalized, 
	%and if it is normalized by SPM99 generate the .mat file for it
	%------------------------------------------------------------------
	if isfield(V,'wfu') & ~isempty(V.wfu),
		Vnew = V;
	else 
		img_fname = fullfile(pth,[nam, '.img']);
		if exist(V.fname)==2, img_fname = V.fname; end
		Vnew = spm_vol(img_fname);
	end
	mat = Vnew.wfu.mat;
	M = Vnew.wfu.M;
	wfu = Vnew.wfu;
	wfu.flip = spm_flip_analyze_images;
	save(fname,'mat','M','wfu');
        %disp(['Saved ' fname]);
end
