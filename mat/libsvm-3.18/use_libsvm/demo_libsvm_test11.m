...
% Load training data
dirData = './data';
load(fullfile(dirData,'ring_3Class_train'));
...
% Load test data set
dirData = './data';
load(fullfile(dirData,'ring_3Class_test'));
...
optionCV.Nlimit = 20;
optionCV.svmCmd = '-q -t 3';
Ncv_param = 3; % Ncv-fold cross validation cross validation

% Put the kernel Phi(data)
PhiEvalData = [(1:size(evalData,1))' evalData*evalData'];
[bestc, bestg, bestcv] = automaticParameterSelection(evalLabel, PhiEvalData, Ncv_param, optionCV);
...
% bestParam = ['-q -t 3 -c ', num2str(bestc), ', -g ', num2str(bestg)];
bestParam = ['-q -t 0'];
% PhiTrainData = [(1:size(trainData,1))' trainData*evalData'];
PhiTrainData = [(1:size(trainData,1))' trainData*trainData'];
model = ovrtrainBot(trainLabel, PhiTrainData, bestParam);
% classify the test data set based on the svm model
% PhiTestData = [(1:size(testData,1))' testData*evalData'];
PhiTestData = [(1:size(testData,1))' testData*trainData'];
[predict_label, accuracy, decis_values] = ovrpredictBot(testLabel, PhiTestData, model);
