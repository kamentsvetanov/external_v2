function [vec_sprime,resultat,ap1,ap2,al]=ml2frontier(in,position,temps,precision_exp,precision_approx,graphe_approx,barre_calcul,graphe_frontiere,trace,write_ecarts,frontier_or_spectrum);
% C'est une procedure qui estime la frontiere microlocale d'une fonction
% en un point, par une methode developpee dans un article Seuret-Levy Vehel
%
% f est la fonction a etudier, sous forme de vecteur de longueur N.
% position est l'abscisse du point a etudier(c'est un nombre entre 1 et N)
% temps est le vecteur temps correspondant au support de la fonction f
%
...
%%%%%%%%%%%% Matrice des (xi-x_position)
B=(temps(position))*ones(N,N);
X1=abs(A-B)+0.0001;

%%%%%%%%%%%% Matrice des (x_position-xj)
A=repmat(temps,1,N);
X2=abs(B-A)+0.0001;
