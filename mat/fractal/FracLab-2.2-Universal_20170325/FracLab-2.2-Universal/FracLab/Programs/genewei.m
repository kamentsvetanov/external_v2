function wei = genewei(N,H,varargin)
% GENEWEI Generates a Weierstrass function
%
%   WEI = GENEWEI(N,H) Generates the Weierstrass function, WEI, using a
%   sample size, N, and a Holder exponent H. The parameter N is a positive
%   integer and the parameter H is a real in (0,1).
%
%   WEI = GENEWEI(...'METHOD') Generates WEI using a specific method. The 
...
%
%   WEI = GENEWEI(...,'support',TMAX) Generates WEI using a specific size of
%   time support, TMAX. The parameter TMAX is a positive integer. 
%   If SUPPORT is not specified, the default value is TMAX = 1.
%
%   WEI = GENEWEI(...,'frequency',L) Generates WEI using a specific multiplicative
%   frequency ratio, L. The parameter L is a positive real superior to 1. 
%   If FREQUENCY is not specified, the default value is L = 2.
%
%   WEI = GENEWEI(...,'nterms',K) Generates WEI using a specific number of
...
%
%   WEI = GENEWEI(...,'seed',S) Generates a stochastic WEI with a specific
%   random seed, SEED. This is useful to generate the same path several times
%   or to compare the paths of different stochastic WEIs. 
%   
%   Example
%
...
%       x = genewei(N,H);
%       figure; plot(t,x);
%       title ('Stochastic Weierstrass function with H = 0.7'); xlabel ('time');
%
%   See also genegwei
%
% Reference page in Help browser
%     <a href="matlab:fl_doc genewei ">genewei</a>
