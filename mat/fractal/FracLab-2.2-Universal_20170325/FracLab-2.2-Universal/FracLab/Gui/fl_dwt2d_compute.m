function [varargout]=fl_dwt2d_compute(varargin)
...
[objTmp,dwt2d_fig] = gcbo;

if ((isempty(dwt2d_fig)) || (~strcmp(get(dwt2d_fig,'Tag'),'Fig_gui_fl_dwt2d')))
    dwt2d_fig = findobj ('Tag','Fig_gui_fl_dwt2d');
end;
...
switch(varargin{1});
    case 'refresh'
        ...
        N = min(size(SigIn)); 
        set(findobj(dwt2d_fig,'Tag','EditText_input'),'String',SigIn_name);
        set(findobj(dwt2d_fig,'Tag','EditText_octaves'),'String',round(log2(N)));
        set(findobj(dwt2d_fig,'Tag','Check_modulus'),'Value',0);
        
    case 'edit_octaves'
        octaves_value = str2double(get(gcbo,'String'));
        SigIn_name = get(findobj(dwt2d_fig,'Tag','EditText_input'),'String');
        eval(['global ' SigIn_name]);
        SigIn = eval(SigIn_name);
        N = min(size(SigIn)); 
    ...
    case 'compute'
        ...
        if isempty(SigIn_Name)
            ...
            set(findobj(dwt2d_fig,'Tag','Button_close'),'enable','off');
            set(findobj(dwt2d_fig,'Tag','Button_compute'),'enable','off');
            
            %%%%% Get Frequency, Voices and Wave %%%%%
            Noctav = str2double(get(findobj(dwt2d_fig,'Tag','EditText_octaves'),'String'));
            
            %%%%% Get wave order %%%%%
            wave_type = get(findobj(dwt2d_fig,'Tag','PopupMenu_wavelet'),'Value');
            switch wave_type
                case 11
            ...
            %%%%% Look for option %%%%%
            Hldabs = get(findobj(dwt2d_fig,'Tag','Check_modulus'),'Value');
            
            %%%%% Perform the computation %%%%%
            OutputNamet = ['dwt2d_' SigIn_Name]; OutputNamemt = ['mwt2d_' SigIn_Name];
            [varnamet varnamemt] = fl_find_mnames(varargin{2},OutputNamet,OutputNamemt);
            eval(['global ' varnamet]);
            varargout{1} = varnamet;
            chaine_in = ['=FWT2D(',SigIn_Name,',',num2str(Noctav),',Q,0,1'];
            ...
            set(findobj(dwt2d_fig,'Tag','Button_close'),'enable','on');
            set(findobj(dwt2d_fig,'Tag','Button_compute'),'enable','on');
        end
    ...
    case 'close'  
        close(findobj('Tag', 'Fig_gui_fl_dwt2d'));
