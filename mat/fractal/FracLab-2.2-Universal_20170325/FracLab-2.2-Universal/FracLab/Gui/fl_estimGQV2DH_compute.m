...
% Callback functions for estimGQV2DH GUI Environment.
...
switch(varargin{1});
    case 'refresh'
        ...
        set(findobj(estimGQV2DH_fig,'Tag','EditText_input'),'String',SigIn_name);

    case 'edit_gamma'
        gamma_value = str2double(get(gcbo,'String'));
        if isnan(gamma_value)
            fl_warning('Gamma must be a real !'); pause(.3);
            set(gcbo,'String',0.8);
        else
    ...
    case 'edit_delta'
        ...
        if isnan(delta_value)
            fl_warning('Delta must be a real !'); pause(.3);
            set(gcbo,'String',1);
        else
    ...
    case 'edit_zones'
        ...
        if isnan(zones)
            ...
            SigIn_name = get(findobj(estimGQV2DH_fig,'Tag','EditText_input'),'String');
            eval(['global ',SigIn_name]);
    ...
    case 'edit_k1'
        ...
        if isnan(k_value)
            ...
            k2 = str2double(get(findobj(estimGQV2DH_fig,'Tag','EditText_k2'),'String'));
    ...
    case 'edit_k2'
        ...
        if isnan(k_value)
            ...
            k1 = str2double(get(findobj(estimGQV2DH_fig,'Tag','EditText_k1'),'String'));
    ...
    case 'check_time'
        if get(gcbo,'Value')
            ...
            set(findobj(estimGQV2DH_fig,'Tag','StaticText_timeinstant'),'enable','off');
            set(findobj(estimGQV2DH_fig,'Tag','EditText_timeinstant'),'enable','off');
            set(findobj(estimGQV2DH_fig,'Tag','StaticText_zones'),'enable','on');
            set(findobj(estimGQV2DH_fig,'Tag','EditText_zones'),'enable','on');
            set(findobj(estimGQV2DH_fig,'Tag','Check_regression'),'enable','on');
        else
            set(gcbo,'String','Single time exponent');
            set(findobj(estimGQV2DH_fig,'Tag','StaticText_timeinstant'),'enable','on');
            set(findobj(estimGQV2DH_fig,'Tag','Regtype_menu'),'enable','on');
            set(findobj(estimGQV2DH_fig,'Tag','EditText_timeinstant'),'enable','on');            
            set(findobj(estimGQV2DH_fig,'Tag','Check_regression'),'enable','off');
            set(findobj(estimGQV2DH_fig,'Tag','StaticText_zones'),'enable','off');
            set(findobj(estimGQV2DH_fig,'Tag','EditText_zones'),'enable','off','String',7);            
        end
    ...
    case 'edit_time'
        ...
        elseif size(time,1) > 1 || size(time,2) ~= 2
            ...
            time = [floor(trunc(time(1),1,N1)),floor(trunc(time(2),1,N2))];
            set(gcbo,'String',[num2str(time(1)),',',num2str(time(2))]);
        end;
    ...
    case 'check_regression'
        if get(gcbo,'Value')
            ...
            set(findobj(estimGQV2DH_fig,'Tag','RadioButton_linear'),'enable','on');
            set(findobj(estimGQV2DH_fig,'Tag','RadioButton_geometric'),'enable','on');
            set(findobj(estimGQV2DH_fig,'Tag','StaticText_zones'),'enable','on');
            set(findobj(estimGQV2DH_fig,'Tag','EditText_zones'),'enable','on');            
            set(findobj(estimGQV2DH_fig,'Tag','StaticText_k1'),'enable','on');
            set(findobj(estimGQV2DH_fig,'Tag','EditText_k1'),'enable','on');
            set(findobj(estimGQV2DH_fig,'Tag','StaticText_k2'),'enable','on');
            set(findobj(estimGQV2DH_fig,'Tag','EditText_k2'),'enable','on');
            set(findobj(estimGQV2DH_fig,'Tag','StaticText_step'),'enable','on');
            set(findobj(estimGQV2DH_fig,'Tag','EditText_step'),'enable','on');
            set(findobj(estimGQV2DH_fig,'Tag','Check_timeinstant'),'enable','on');
            set(findobj(estimGQV2DH_fig,'Tag','Regtype_menu'),'enable','on');
        else
            set(gcbo,'String','No Regression');
            set(findobj(estimGQV2DH_fig,'Tag','RadioButton_linear'),'enable','off');
            set(findobj(estimGQV2DH_fig,'Tag','RadioButton_geometric'),'enable','off');
            set(findobj(estimGQV2DH_fig,'Tag','StaticText_zones'),'enable','off');
            set(findobj(estimGQV2DH_fig,'Tag','EditText_zones'),'enable','off','String',7);            
            set(findobj(estimGQV2DH_fig,'Tag','StaticText_k1'),'enable','off');
            set(findobj(estimGQV2DH_fig,'Tag','EditText_k1'),'enable','off');
            set(findobj(estimGQV2DH_fig,'Tag','StaticText_k2'),'enable','off');
            set(findobj(estimGQV2DH_fig,'Tag','EditText_k2'),'enable','off');
            set(findobj(estimGQV2DH_fig,'Tag','StaticText_step'),'enable','off');
            set(findobj(estimGQV2DH_fig,'Tag','EditText_step'),'enable','off');
            set(findobj(estimGQV2DH_fig,'Tag','Check_timeinstant'),'enable','off');
            set(findobj(estimGQV2DH_fig,'Tag','Regtype_menu'),'enable','off');
        end
    ...
    case 'compute'
        ...
        if isempty(SigIn_Name)
            ...
            %%%%% Get Gamma and Delta %%%%%
            Gamma = str2double(get(findobj(estimGQV2DH_fig,'Tag','EditText_gamma'),'String'));
            Delta = str2double(get(findobj(estimGQV2DH_fig,'Tag','EditText_delta'),'String'));
            
            %%%%% Get Initial, Final and Step of Sampling %%%%%
            zones = str2double(get(findobj(estimGQV2DH_fig,'Tag','EditText_zones'),'String'));
            k1 = str2double(get(findobj(estimGQV2DH_fig,'Tag','EditText_k1'),'String'));
            k2 = str2double(get(findobj(estimGQV2DH_fig,'Tag','EditText_k2'),'String'));
            Step = str2double(get(findobj(estimGQV2DH_fig,'Tag','EditText_step'),'String'));
            
            %%%%% Look for option %%%%%
            Hldgeo = get(findobj(estimGQV2DH_fig,'Tag','RadioButton_geometric'),'Value');
            Hldreg = get(findobj(estimGQV2DH_fig,'Tag','Check_regression'),'Value');                    
            Hldtimeinstant = get(findobj(estimGQV2DH_fig,'Tag','Check_timeinstant'),'Value');
            timeinstant = str2num(get(findobj(estimGQV2DH_fig,'Tag','EditText_timeinstant'),'String')); %#ok<ST2NM>
            Hldzones = isequal(zones,7);
            
            %%%%% Regression argument %%%%%
            RegType=get(findobj(estimGQV2DH_fig,'Tag','Regtype_menu'),'Value');
            scale = k1:Step:k2;
            ...
            chaine_in = ['=estimGQV2DH(',SigIn_Name,',',num2str(Gamma),',',num2str(Delta),...
            ...
            if ~Hldreg
                chaine_in = ['=estimGQV2DH(',SigIn_Name,',',num2str(Gamma),',',num2str(Delta),',1,','''noreg'''];
            end
            ...
            if ~Hldtimeinstant
                ...
                chaine = [varnameH chaine_in,',''timeinstant'',[',num2str(timeinstant(1)),',',num2str(timeinstant(2)),']'];
            end
    ...
    case 'close'  
        close(findobj('Tag', 'Fig_gui_fl_estimGQV2DH'));
