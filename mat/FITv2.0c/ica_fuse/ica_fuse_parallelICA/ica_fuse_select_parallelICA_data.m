...
if isempty(inputFile)
    ...
    if (length(featureNamings) ~= 2)
        error('Number of features you can enter is 2');
    end
    
    if (length(modalities) ~= 2)
        error('Number of modalities you can enter is 2');
    end
...
if (dataSelectionOption == 2)
    ...
    if isempty(inputFile)
        ...
        tmp_files = ica_fuse_ui_select_data('title', 'Select data for fusion', 'num_subjects', ...
            numGroups, 'num_sessions', numFeatures, 'filter_string', '*.img;*.asc', 'type_file_selection', 'multiple', ...
