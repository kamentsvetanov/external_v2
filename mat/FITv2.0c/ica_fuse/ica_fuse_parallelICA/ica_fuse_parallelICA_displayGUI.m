...
displayParameters = struct('outputDir', outputDir, 'output_prefix', output_prefix, 'dataInfo', dataInfo, 'groupNames', groupNames, ...
    ...
    'numSubjects', numSubjects, 'numComp', numComp, 'avg_correlation', avg_correlation, 'corrIndices', paraICAInfo.run_analysis.corrIndices, ...
...
try
    ...
    if (strcmpi(sortComponentsStr, 'correlation between modalities'))
        ...
        if (~isempty(geneModality) && ~isempty(locusFileName) && strcmpi(type_parallel_ica, 'aa'))
    ...
    for nFeature = 1:length(selectedFeatureVal)
        ...
        elseif strcmpi(checkModality, 'gene')
        ...
        for nComp = 1:length(currentComps)
            ...
            elseif strcmpi(checkModality, 'eeg')
                ...
                if (nComp == 1)
                    ...
                    meanDataEEG = mean(squeeze(meanDataEEG(:, 2, :)), 2);
            ...
            if nFeature == 1
                if (strcmpi(sortComponentsStr, 'correlation between modalities'))
                    axesTitle = ['Mixing Coefficients (Correlation = ', num2str(sorted_values(1).val(nComp)), ')'];
                elseif (strcmpi(sortComponentsStr, 'two sample t-test on mixing coefficients'))
