function [clpos, clneg, clpos_data, clneg_data, clpos_data2, clneg_data2] = mediation_brain_results(meth, varargin)
    ...
    %
    % 1) Get results for brain mediation search
    % 2) Display on orthviews
    % 3) Extract data from contiguous regions (blobs) and return 'clusters'
    %       structure with contiguous regions saved
    % 4) Print tables of clusters
    % 5) Show montages of slices
    %
    % Tor Wager, Feb 07
    % Update: July 07, works with robfit directories too!
    %
    % meth options
    % Specify analysis package-specific effects
    % --------------------------------------------
    ...
    function     [pruneopt, addopt, overlay, meth, thresh, szthresh, mask, ...
        ...
        switch lower(meth)
            ...
            case 'rob0'
                analysistype = 'robfit';
                pname = 'rob_p_0001.img';
                effname = 'rob_tmap_0001.img';
                Z_descrip2 = 'Robust regression intercept.';

            case 'rob1'
                analysistype = 'robfit';
                pname = 'rob_p_0002.img';
                effname = 'rob_tmap_0002.img';
                Z_descrip2 = 'Robust regression predictor.';
                
            case 'rob2'
                analysistype = 'robfit';
                pname = 'rob_p_0003.img';
                effname = 'rob_tmap_0003.img';
                Z_descrip2 = 'Robust regression predictor.';    

            case 'rob3'
                analysistype = 'robfit';
                pname = 'rob_p_0004.img';
                effname = 'rob_tmap_0004.img';
                Z_descrip2 = 'Robust regression predictor.'; 
