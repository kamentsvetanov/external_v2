...
% Added by kat35 to load screenshot
if nargin<1
    p = fileparts(mfilename('fullpath'));
%     imshow(fullfile(p,'screenshot.png'));
    [YourImage, ~, ImageAlpha] = imread(fullfile(p,'screenshot.png'));
...
% Added by KAT Neuroconda needed for pycolormat, BUT needs to be removed for ICA-AROMA
% unix('source /imaging/local/software/neuroconda/v2_0/neuroconda.csh');
...
% kat added neuroconda
comd = sprintf('sneuroconda; %s "%s" %s -o "%s" -n %d', pyCmd, pyScript, nam, tmpf, n);
[s,m] = system(comd);
