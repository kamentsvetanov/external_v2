...
% ______________________________________________________________________
% $Id: cat_spm_preproc_run.m 1791 2021-04-06 09:15:54Z gaser $
    ...
    for subj=1:numel(job.channel(1).vols)
        ...
        if iter==nit
            ...
            if any(job.Yclsout) %% ADDED RD
              [vout.Ym,vout.Ycls] = cat_spm_preproc_write8(res,tmp1,tmp2,tmp3,job.warp.mrf,job.warp.cleanup,job.warp.bb,job.warp.vox,job.Yclsout);
            else
              vout.Ym = cat_spm_preproc_write8(res,tmp1,tmp2,tmp3,job.warp.mrf,job.warp.cleanup,job.warp.bb,job.warp.vox);
            end
