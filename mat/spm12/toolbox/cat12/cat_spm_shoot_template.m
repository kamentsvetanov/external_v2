function out = cat_spm_shoot_template(job)
%cat_spm_shoot_template. Like spm_shoot_template but definable def-file
%
...
% ______________________________________________________________________
% $Id: cat_spm_shoot_template.m 1791 2021-04-06 09:15:54Z gaser $

%_______________________________________________________________________

d       = spm_shoot_defaults;
% ######### CAT12.begin
if isfield(job,'dfile')
  % use own default file, print error if it cannot be seen by MATLAB 
  d = struct(); 
  [pp,ff,ee] = spm_fileparts(job.dfile{1}); 
  if ~exist(ff,'file'), error('The given file "%s" does not exist (in the MATLAB patch).\n',job.dfile{1}); end
  eval(sprintf('d = %s;',ff)); 
  clear pp ff ee; 
  
  % for the longitudinal processing we need unique filenames 
  [pp,ff] = spm_str_manip(job.images{1},'trC'); 
  d.tname = strrep(d.tname,'FNAME',[ff.s(3:end) 'XXX' ff.e]); 
  clear pp ff; 
  
  % adapt schedule for resolution
  V        = spm_vol(job.images{1}{1});
  vx_vol   = sqrt(sum(V.mat(1:3,1:3).^2));  
  d.sched  =  d.sched ./ prod(vx_vol);
end
% ######### CAT12.end
