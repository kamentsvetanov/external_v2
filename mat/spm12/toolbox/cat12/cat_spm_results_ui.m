...
% ______________________________________________________________________
% $Id: cat_spm_results_ui.m 1880 2021-09-28 12:04:36Z gaser $
...
elseif nargin == 1 && isstruct(varargin{1})
  cat_spm_run_results(varargin{1}); 
  varargout = {[],[],[]};
  return
else
  Action=varargin{1}; 
...
%==========================================================================
function out = cat_spm_run_results(job)
...
for k = 1:numel(cspec)
    ...
    %----------------------------------------------------------------------
    if ~spm('CmdLine')
        [hReg, xSPM, SPM] = cat_spm_results_ui('Setup',xSPM);
                        ...
                        switch lower(job.view)
                          ...
                          otherwise
                            if isnumeric(job.view) && size(job.view)==2
                              ...
                              cat_io_cprintf('err','Unknown view.\n');
                            end
                        end
                      end
                      
                      job.imgprint.type   = fn(1:3);
                      if numel(fn) > 3
                        job.imgprint.dpi  = round(str2double(fn(4:end)));
                      else
                        job.imgprint.dpi  = 300;
                      end
                      job.imgprint.fdpi   = @(x) ['-r' num2str(x)];
                      job.imgprint.ftype  = @(x) ['-d' strrep( x , 'jpg' , 'jpeg' ) ];
                      job.imgprint.fname  = fullfile( spm_fileparts(job.spmmat{1}), sprintf('catresults_con%d-%s_%s%0.0e_ex%d_conj%d_msk%d.%s.%s',...
                        job.conspec.contrasts, job.conspec.titlestr, ...
                        job.conspec.threshdesc, job.conspec.thresh,  ...
                        job.conspec.extent, job.conspec.conjunction, ...
                        ~isfield(job.conspec.mask,'none'), viewname, job.imgprint.type)); 
                      
                      
                      if 0 %exist('exportgraphics','file')
                        exportgraphics(fg,job.imgprint.fname,'Resolution',job.imgprint.dpi)
                      else
                        % paperposition defines fontsize and we compensate maybe the screen resolution of 0.75 dpi ...
                        set(fg,'PaperPositionMode','manual','resize','off','PaperPosition',[0 0 1.4 1.4]); 
                        print(fg, job.imgprint.ftype(fn), job.imgprint.fdpi(job.imgprint.dpi), job.imgprint.fname);
                      end
                    otherwise
