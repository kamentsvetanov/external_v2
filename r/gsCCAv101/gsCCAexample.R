#
# Examples of how to use the gsCCA code
#
...
source("/imaging/camcan/sandbox/kt03/projects/external/r/gsCCAv101/gsCCAtools.R")
...
# model. Note that gsCCAtools.R has a function for sampling data
...
print("==================")
model <- gsCCAexperiment(Y,K,opts)
...
# to see how gsCCA gives better predictions for small sample sizes.
#
...
for(m in 1:2) {
  ...
  trimmed <- gsCCAtrim(model)
  pred2 <- gsCCApred(observed,Ytest,trimmed)
  error2 <- vector()
  for(d in 1:D[mpred]) {
  ...
  X11();
  barplot(rbind(error,error2,error3),beside=TRUE,legend.text=c("gsCCA",
                                                   "trimmed gsCCA",
                                                   "Regression"))
...
print("========================================")
data <- gsCCAsample(trimmed,Ntrain)
newmodel <- gsCCAexperiment(data$Y,K,Nrep=3,opts)
newtrimmed <- gsCCAtrim(newmodel)
